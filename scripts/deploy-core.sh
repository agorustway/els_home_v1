...
echo ">>> Build els-core image..."
$SUDO_BIN PATH="$DOCKER_PATH" "$COMPOSE_BIN" -f docker/docker-compose.yml build els-core

echo ">>> Remove existing els-core container..."
if $SUDO_BIN PATH="$DOCKER_PATH" "$DOCKER_BIN" container inspect els-core >/dev/null 2>&1; then
    $SUDO_BIN PATH="$DOCKER_PATH" "$DOCKER_BIN" rm -f els-core
fi

echo ">>> Recreate els-core..."
$SUDO_BIN PATH="$DOCKER_PATH" "$COMPOSE_BIN" -f docker/docker-compose.yml up -d --no-build --force-recreate els-core
