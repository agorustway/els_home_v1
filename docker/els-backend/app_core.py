...
def get_vehicle_tracking():
    ...
    if vehicle_numbers:
        try:
            drivers = supabase.from_("driver_contacts").select("vehicle_number, contract_type, cargo_type, map_visibility, general_vehicle_type, general_payload, general_body_type").in_("vehicle_number", vehicle_numbers).execute().data or []
            for d in drivers:
