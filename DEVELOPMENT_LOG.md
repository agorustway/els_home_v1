...
## [2026-01-23] - 인트라넷 내비게이션 개편 및 자료실 UI 개선

### 🚀 핵심 성과
- **인트라넷 메뉴 구조 혁신**: 기존의 세로형 사이드바를 제거하고, '회사소개'와 동일한 가로형 미들바(Middle Bar) 내비게이션으로 전면 교체하여 사용성을 대폭 강화했습니다.
- **모바일 뷰 최적화**: 미들바 메뉴가 적을 때는 중앙 정렬로 미학적 완성도를 높이고, 메뉴가 늘어나면 자동으로 슬라이드(내부 스크롤)가 작동하도록 유동적인 레이아웃을 구현했습니다.
- **자료실(NAS) 사용자 경험 개선**: NAS 연결 지연 시 노출되는 로딩 메시지를 친절하고 명확한 안내 문구로 수정하여 사용자의 불안감을 해소했습니다.

### 🛠️ 작업 상세
#### 1. 내비게이션 시스템 개편
- `IntranetSubNav.js` & `module.css`: 데스크탑에서도 항상 노출되도록 변경하고, '내 업무보고', '권한관리', '문의관리' 등 사이드바에 있던 모든 메뉴를 통합했습니다.
- **로그아웃 통합**: 사이드바에 있던 로그아웃 기능을 미들바 우측(데스크탑 전용)으로 배치하여 접근성을 유지했습니다.
- `EmployeeSidebar.js`: 더 이상 사용하지 않으므로 전사 레이아웃에서 제거했습니다.
- `layout.js` & `page.js`: (intranet) 그룹 및 어드민, 지점별 페이지에서 사이드바 의존성을 제거하고 본문 영역을 확장했습니다.

#### 2. 모바일 반응형 강화
- `SubNav.module.css` & `IntranetSubNav.module.css`: `@supports (justify-content: safe center)`와 `overflow-x: auto` 조합을 통해, 메뉴 개수에 상관없이 최적의 정렬과 스크롤 경험을 제공하도록 고도화했습니다.

#### 3. 자료실 로딩 고도화
- `ArchiveBrowser.js`: "자료실은 NAS 내부 서버와의 연결로 로딩 속도가 느립니다. 잠시만 기다려 주세요." 문구 적용 및 로딩 UI 가독성 개선.

### 📋 참고 사항
- **사이드바 제거**: 이제 인트라넷 내의 모든 이동은 상단 미들바를 통해 이루어집니다. 본문 영역이 넓어져 데이터 확인이 더 쉬워졌습니다.
- **슬라이더 동작**: 모바일에서 메뉴가 화면 너비를 넘어가면 자연스럽게 좌우 스크롤(슬라이드)이 가능합니다.
