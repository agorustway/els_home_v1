...
- 웹 UI: `AsanAnnualPerformance`
  ...
  - 연간 분석 컨테이너는 `annualAnalytics` 전용 세로 레이아웃과 `stretch` 폭 고정을 사용해 월간실적 `.analytics` 카드 그리드와 분리한다.
  - 분석 하위 탭: `개요`, `연도×월`, `계약/차량`, `주차·요일`, `검증·근거`. 원장 장기 흐름은 개요 상단에 고정해 별도 `10년 흐름` 탭을 두지 않는다.
  - 분석 축: 연간 성과 리포트, 이익 구조, 성과 경보, 연도별·월별 흐름, 연도×월 히트맵, 주차/요일 흐름, 검증/근거 설명
  - 회계 분석 축: 매출(`청구`), 매입(`하불`), 이익, 이익률, 원가율, 고객/작업지/운송사/노선/구분별 공헌도와 상위 집중도
  - `운송사(명의)=ELS솔루션`은 외부 운송사 비교 대상과 분리한다.
  - `ELS솔루션+직계약`은 우리 직계약차량 세그먼트로 별도 집계하고, 작업지/청구처/노선/구분 상세 버튼은 AND 검색으로 원장 테이블에 연결한다.
  - 계약/차량 월별 흐름 차트는 매출/이익 선 외에 평균선, 최고/최근 포인트, 평균 이익률 요약 카드를 함께 표시한다.
  - 차량별 이익은 실제 `영업넘버`만 랭킹으로 보여주며, 빈칸/`-` 영업넘버는 `차량번호 미기재` 품질 지표로 별도 표시한다.
  ...
  - `구간단가`는 연간/월간 총액 리포트와 분리된 별도 탭이다. 운영 기준은 월간 마감자료 current 원장만 사용한다. 36만 행 연간 원장까지 합산하면 화면 목적 대비 DB 부하가 커지고, TYPE/청구픽업/선적 조건 신뢰도도 월간 원장보다 낮아진다.
  - 구간단가 조회 범위는 `전체/연도별/월별`이며, 컬럼 제목열 안의 필터 버튼과 제목열 정렬은 화면에 로드된 전체 금액표 묶음 기준으로 동작한다.
  - 구간단가 묶음 항목은 화면에서 제외/포함할 수 있다. 제외된 항목은 집계 키에서 빼고 같은 `청구/하불 금액 + 남은 조건`을 한 줄로 다시 압축한다.
  - 구간단가 묶음 기준은 `청구/하불 금액 + 매출, 지역, 작업지, 운송사, 구분, 픽업, 청구픽업, 선적, TYPE, 청구처, 하불처`다. 화면은 단가 차트가 아니라 금액 확인용 테이블이며 필터/정렬 중심으로 운영한다.
...
- 원장 행은 마감월 기준 월별 흐름과 작업일자 기준 일별 흐름을 함께 집계한다. `청구/하불`은 해당 마감월 반영 금액이고, 첫 컬럼 값이 `이월`인 행은 전월에서 넘어와 이번 달에 반영된 `청구이월 반영분`으로 분리한다.
- `이월구분 + 청구_1/하불_1`은 이번 마감에서 다음 마감월로 넘길 `익월이월 발생분`으로 분리한다. 화면은 `청구이월`과 `익월이월`을 별도 KPI로 표시하고, 월별 summary에는 `carryoverCycle.incoming/outgoing` 및 월별 `incomingCarryover*`/`carryover*` 필드를 보존한다.
...
- 실제 엑셀 샘플 기준으로 매출/매입/이익 컬럼 자동 추론 키워드 보정.
