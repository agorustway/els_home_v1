...
- 원장 행은 마감월 기준 월별 흐름과 작업일자 기준 일별 흐름을 함께 집계한다. `청구/하불`은 해당 마감월 반영 금액이고, 첫 컬럼 값이 `이월`인 행은 전월에서 넘어와 이번 달에 반영된 `상단이월 반영분`으로 분리한다.
- `이월구분 + 청구_1/하불_1`은 이번 마감에서 다음 마감월로 넘길 `익월이월 발생분`으로 분리한다. 화면은 `상단이월`과 `익월이월`을 별도 KPI로 표시하고, 월별 summary에는 `carryoverCycle.incoming/outgoing` 및 월별 `incomingCarryover*`/`carryover*` 필드를 보존한다.
