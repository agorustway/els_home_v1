...
- **날씨 페이지 데이터 정밀화**: 아산 시청 중심 좌표(`36.7898, 127.0018`) 적용 및 타임존 인식 로직 수정을 통해 현지 기상청 데이터와 정확도 일치.
- **메뉴 진입 안정화**: 직접 URL 이동 후 대기 시간을 15초로 연장하여 WebSquare의 느린 렌더링 환경에서도 실패 없이 진입하도록 개선.
