...
## 📅 2026-02-25: 컨테이너 이력조회 최적화 및 ETRANS 엔진 안정성 강화

### 핵심 성과
- **컨테이너 이력조회 최적화**: 로그 렌더링 렉 현상을 해결하기 위해 `logLines` 상태를 최신 100개로 제한.
- **ETRANS 엔진 에러 수정**: 메뉴 진입 시 잘못된 URL(/main/)로 인한 404 에러를 수정하고, 로그인 시 엔터 입력 및 JavaScript 클릭을 병행하여 성공률 향상.
- **안전운임 비우기 강화**: 초기화 시 주소 및 세션 스토리지 정보를 포함한 모든 잔여 데이터 제거 로직 보강.
- **모바일 UX 개선**: 컨테이너 이력조회 페이지에 모바일 전용 카드 뷰 스타일 적용.

### 변경 파일
- `web/app/employees/container-history/page.js`
- `web/app/employees/container-history/container-history.module.css`
