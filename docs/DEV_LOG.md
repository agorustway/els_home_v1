...
## 📅 2026-02-26: NAS 리소스 및 타임아웃 2차 안정화

### 핵심 성과
- **크롬 경로 자동 탐색**: `CHROME_BIN` 환경 변수 및 `/usr/bin/chromium` 등 NAS 환경 경로 자동 인식 로직 추가.
- **타임아웃 조율**: 백엔드(400s)와 데몬(350s) 간 타임아웃 차등 적용으로 응답 불일치 해결.
- **실시간 디버그 로그**: 브라우저 생성 단계별 로그 추가로 '먹통' 구간 추적 강화.
- **리소스 문답**: 로그 폴링 방식이 메모리 버퍼(deque) 기반임을 확인하여 시스템 부하 영향 없음을 검증.
- **모바일 UI 최적화 (S24)**: 컨테이너 이력조회 페이지의 로그인 섹션, 검색 필터, 푸터 레이아웃을 반응형으로 개선하여 가로 넘침 현상 해결.
