...
### 원인
- 예측 손익 카드가 기간 요약 카드 바로 아래에 있어, 하단 날짜/기간 선택에서 바뀌는 값이라는 연결성이 약했습니다.
- 배차 원장에 `부산북항`, `부산신항`, `신항`, `인천여객`, `인천신항`, `인천북항`, `기타/철송` 같은 세부 상차지명이 늘면서 상차지별 비율에 대표 지역이 중복 노출되었습니다.
- 예측 단가 매칭은 작업지 후보가 어긋나면 화주/포트가 유사해도 평균 단가로 빠질 수 있어, `울산 서영`, 부산/인천 세부 포트처럼 사람이 보면 같은 축인 케이스를 놓칠 수 있었습니다.

### 조치
- `예측 손익` 패널을 일자별 추세/요일별 비교와 날짜 선택 블록 다음으로 이동해, 하단 날짜 선택이 해당 카드에도 적용된다는 흐름을 맞췄습니다.
- 상차지 파싱은 원본 헤더를 보존하되 화면 집계명만 정규화하도록 바꿔 `부산북항/부산신항/북항/신항/이지B -> 부산`, `인천여객/인천신항/인천북항/인천국제 -> 인천`, `기타/철송/철송 -> 기타`로 합산했습니다.
- 예측 단가 매칭 정규화에 부산/인천 세부 포트 alias를 추가하고, 작업지 불일치만으로 후보를 버리지 않도록 조정했습니다. 단, 화주/포트/카테고리 같은 강한 단서가 2개 이상일 때만 실제 단가 후보로 인정해 오매칭 위험은 제한했습니다.

### 검증
- `node --test --test-name-pattern "예측 손익|상차지별|모바일 날짜" web/tests/asanDashboardView.test.mjs`: 7개 통과
- `cd web; npm.cmd run lint`: 통과
- 참고: 전체 `node --test web/tests/asanDashboardView.test.mjs`는 현 작업과 별도인 GLAPS 마스터 정적 기대값(`[fetchData, refreshToken]`) 1건 불일치로 48/49 통과 상태입니다.
...
- 현황판 캐시 GET은 원장 meta 서명과 저장된 `source_signature`가 다르면 full 원장을 다시 읽어 캐시를 즉시 재생성하도록 보강했습니다.
...
### 원인
- `2026년_배차-일일배차(모비스AS).xlsm`의 날짜 시트 업체별 합계 배열수식 스필/캐시가 일부 업체까지만 저장되어 있었습니다.
- `6.5` 시트는 상단 `오더량/배차량 6`인데 하단 업체별 합계가 `이지=4` 한 줄만 남아 4대로 표시됐고, `6.4` 시트도 상단 배차량 25 대비 업체별 합계가 23대로 표시됐습니다.

### 조치
- NAS 최신 원본을 내려받아 Excel COM으로 `6.1/6.2/6.4/6.5` 업체별 합계 영역을 현재 배차칸 범위 기준으로 재입력하고 `CalculateFullRebuild()`를 실행했습니다.
- `6.1/6.2`는 기존 총량 유지, `6.4/6.5`는 누락 업체까지 포함되도록 스필 범위를 확장했습니다.
- 백업: `/volume2/아산지점/A_운송실무/2026년_배차-일일배차(모비스AS).xlsm.backup-summaryrecalc-20260604-172119`

### 검증
- 교체 후 NAS 재다운로드본 SHA256: `69987b1de9dcf0a00ee11587629629a5352a6e816b33dacac70b4c4e5ec3d9ec`
- OOXML 검사: `6.1 B182:C189`, `6.2 B184:C193`, `6.4 B181:C188`, `6.5 B179:C181` 배열수식 확인
- Excel COM 매크로 허용 재오픈: `6.4 C178=25`, `6.5 C176=6`, 상단 `오더량/배차량`과 일치 확인

### 변경 파일
- NAS 원본: `/volume2/아산지점/A_운송실무/2026년_배차-일일배차(모비스AS).xlsm`
- `docs/01_MISSION_CONTROL.md`, `docs/02_DEVELOPMENT_LOG.md`

---

## [2026-06-04] NAS 글로비스KD외 배차 원본 6.5 업체별 합계 전체 재계산

### 원인
- `2026년_배차-일일배차(글로비스KD외).xlsm`의 `6.5` 시트에서 업체별 합계 영역 `B734:C...` 배열수식 스필/캐시가 일부 업체까지만 저장돼 있었습니다.
- 실제 운영 저장본은 `B734:C739`까지만 계산되어 하단 업체별 합계가 50대로 보였고, 같은 시트의 배차량/작업지 합계 62대와 불일치했습니다.

### 조치
- NAS 최신 원본을 다시 내려받아 Excel COM으로 `6.5` 시트 `B734:C751`을 초기화한 뒤 `B734`에 `=GetTextQuantityListFast(O5:W728)`를 `Formula2`로 재입력했습니다.
- `CalculateFullRebuild()` 후 저장한 복구본을 NAS 원본 백업 생성 후 교체했습니다.
- 백업: `/volume2/아산지점/A_운송실무/2026년_배차-일일배차(글로비스KD외).xlsm.backup-fullrecalc-20260604-170851`

### 검증
- 교체 후 NAS 재다운로드본 SHA256: `8acebd5c7301aa9e87e0c5af0e6bf8ebd20e2c709ca26f40893061082c8eef4a`
- OOXML 검사: `6.5` 시트 배열수식 `B734:C747`, 마지막 업체 `CSS=1` 확인
- Excel COM 매크로 비활성 재오픈: `6.5 C733=62`, `G734=62`, 업체별 합계 `자차18/대신12/셔틀7/신승6/칸4/유엘피3/삼익2/선진2/이지2/한진2/경평1/포유1/천일중부1/CSS1` 확인

### 변경 파일
- NAS 원본: `/volume2/아산지점/A_운송실무/2026년_배차-일일배차(글로비스KD외).xlsm`
- `docs/01_MISSION_CONTROL.md`, `docs/02_DEVELOPMENT_LOG.md`
