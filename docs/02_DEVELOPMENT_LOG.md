## [2026-06-04] NAS 글로비스KD외 배차 원본 6.5 업체별 합계 복구

### 원인
- `2026년_배차-일일배차(글로비스KD외).xlsm`의 `6.5` 시트에서 업체별 합계 영역 `B734:C...` 배열수식 메타가 깨져 있었습니다.
- 정상 시트는 `B734:C748`처럼 `=GetTextQuantityListFast(O5:W728)`가 2열 배열수식으로 저장되지만, `6.5`는 `B734` 한 칸 일반 수식으로만 남아 `자차` 텍스트만 보이고 수량열이 비어 합계가 0으로 표시됐습니다.

### 조치
- NAS 원본을 로컬 임시 복사본으로 내려받아 Excel COM으로 `6.5` 시트 `B734:C751`을 초기화한 뒤 `B734`에 `=GetTextQuantityListFast(O5:W728)`를 `Formula2`로 재입력했습니다.
- 재계산/저장 후 NAS 원본을 백업하고 복구본으로 교체했습니다.
- 백업: `/volume2/아산지점/A_운송실무/2026년_배차-일일배차(글로비스KD외).xlsm.backup-arrayfix-20260604-142941`

### 검증
- 교체 후 NAS 재다운로드본 SHA256: `454cd9f546273c67a95653a76c96c87755633b2396b1b74c365e96c099ef937b`
- OOXML 검사: `6.5` 시트 배열수식 `B734:C739`, `C733=23`, `B734=자차`, `C734=11` 확인
- Excel COM 매크로 비활성 재오픈: `Sheets=8`, `FileFormat=52`, `HasVBProject=True`, `6.5 C733=23`, `B735=셔틀`, `C735=5` 확인
