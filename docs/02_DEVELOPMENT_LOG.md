...
- **앱 지도 제어 직관화**: 앱 지도 상단에 핀 모양 아이콘과 텍스트 버튼으로 중복 존재하던 내 위치 찾기 기능을 `📍 내 위치` 텍스트 버튼 1개로 통일하여 깔끔하게 정리했습니다.
- **앱 마커 클릭 액션 변경**: 마커 클릭 시 단순히 위치를 중심으로 이동하던 방식에서, 클릭한 차량의 운행 경로와 상세 정보를 하단 패널에 띄우도록(상세보기 기능과 동일하게) 변경했습니다. 이때 내 위치 자동 추적은 잠시 중단되어 쾌적한 상세 정보 조회가 가능하며, 마커의 부드러운 위치 이동은 백그라운드에서 계속 동작합니다.
