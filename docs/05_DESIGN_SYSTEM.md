...
인트라넷에서 제공하는 엑셀 다운로드는 아산지점 배차판의 `상세배차내역` 다운로드를 기준 톤앤매너로 삼는다. 서버에서 생성하는 데이터형 엑셀은 `web/utils/intranetExcelExport.mjs` 공통 유틸을 우선 사용하고, 클라이언트 fallback 엑셀도 같은 색상과 행 구조를 맞춘다.

### 기준 구현
- 공통 유틸: `web/utils/intranetExcelExport.mjs`
- 기준 화면: `/employees/branches/asan` 상세배차내역 엑셀 다운로드
- 적용 대상: 아산 배차판/선적관리, 안전운임, 차량관제, 연락처/작업지 양식, 컨테이너 이력 fallback 등 인트라넷 엑셀 산출물

### 공통 스타일
- 폰트: `맑은 고딕`, 본문 `10pt`
- 제목 행: 좌측 정렬, `13pt`, 굵게, 텍스트 `#0F172A`
- 생성 정보 행: `9pt`, 텍스트 `#64748B`
- 헤더 행: 배경 `#1F5673`, 흰색 굵은 글자, 중앙 정렬
- 테두리: 헤더 상하 `#16445C`, 헤더 좌우 `#6BA4BC`, 본문 `#D7DEE8`
- 본문 정렬: 텍스트 좌측, 숫자 우측, 모든 셀 수직 가운데
- 숫자 서식: 수량/거리/운임 등 숫자형 컬럼은 `#,##0`
- 틀 고정: 제목/생성 정보가 있으면 실제 헤더 행까지 고정
- 자동 필터: 반복 데이터형 시트는 실제 헤더 행 기준으로 적용

### 컬럼 너비
- 해당 컬럼의 데이터 중 가장 긴 문자열 길이에 맞춘다.
- 한글은 약 `2.1`, 영문/숫자는 약 `1.1` 가중치를 적용한다.
- 최대 너비는 `80`으로 제한한다.

### 데이터형과 보고서형 구분
- 일반 데이터형 엑셀은 제목, 생성 정보, 헤더, 반복 행 구조를 사용한다.
- 안전운임 `구간조회`처럼 지도 경로, 고시 운임, 할증, 운행비가 섞인 화면은 일반 테이블로 억지 변환하지 않는다. 제목, 생성 정보, 요약행, 섹션 헤더, 세부 행을 가진 보고서형 시트로 구성한다.
- 보고서형 시트의 저장 내역이나 반복 목록은 별도 시트에서 일반 데이터형 표로 제공한다.
