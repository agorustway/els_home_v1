...
- **v5.10.42 Metadata**: `driver_contacts` 조인 결과를 반영해 `cargo_type`, `driver_contract_type`, `branch`, `partner_company`, `map_visibility`, `general_vehicle_type`, `general_payload`, `general_body_type`을 함께 반환합니다.
- **Filtering Policy**: 웹은 업무유형(`container`/`general`) 1차 필터와 계약상태(`contracted`/`uncontracted`/`partner`) 2차 필터를 적용합니다. 드라이버 앱 지도는 앱에서 공개범위를 선택하지 않고, 웹 운전원정보의 `map_visibility` 관리 정책을 조회해 자기차량/계약차량/전체운행차량을 선별합니다.
