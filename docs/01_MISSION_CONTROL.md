# ELS MISSION CONTROL (v5.14.355 / APK v5.11.29)

> 최신 업데이트: 아산 현황판 하단 월별/전체 선택이 일별 scope로 고정되던 문제를 수정한다.
...
- 배차 `현황판`은 `branch_dispatch_dashboard_cache`의 집계 payload를 우선 사용한다. 첫 화면은 선택일/현재·기본 주월만 담은 `scope=initial`을 받고, full 스냅샷은 백그라운드로 교체한다. 하단 기간 선택의 일/주/月/전체 모드는 카드·도넛·비중차트 active scope와 일치해야 한다.
...
## VERIFICATION
- 현황판 월별/전체 active scope 수정은 `node --test web/tests/asanDashboardView.test.mjs`로 검증한다.
