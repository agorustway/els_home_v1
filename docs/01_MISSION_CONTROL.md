...
## 📌 현재 상태 및 핵심 지표
- **현재 버전**: `v5.2.1` (NAS Parsing Optimization)
- **핵심 목표**: NAS 모든 문서 파싱 효율화 + 불필요 파일(이메일, 대용량, 자료실) 필터링
- **최근 승전보**: 자료실 및 아산지점 특정 하위폴더 파싱 제외, 20MB 초과 대용량 파일 스킵 로직 적용 (2026-04-22)
...
- [✅] **v5.2.1**: NAS 파싱 필터링 및 최적화 (자료실, 대용량, 특정폴더 제외)
- [ ] **Next**: 사용자별 접근 권한 분리 및 최종 마이그레이션 (Phase 6)

## 🚧 최근 이슈 및 조치 (2026-04-21)
- **[SOLVED] 아산 배차판 자동 업데이트 [Errno -2] 끈질긴 장애 완전 종결**: 
  1) **DNS 3중 그물망 패치**: `httpx` 내부에서 `socket`을 우회하는 현상을 포착, `httpcore`의 최하단 통신 함수를 직접 하이재킹하여 강제 IP 매핑 성공 (`v5.0.47`).
  2) **파일 마운트 정상화**: 도커 컨테이너가 나스의 실제 엑셀 폴더(`/volume2`)를 바라보지 못하던 경로 불일치 문제를 `docker-compose.yml` 수정으로 해결 (`v5.0.48`).
