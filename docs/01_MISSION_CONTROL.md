# ELS MISSION CONTROL (v5.14.285 / APK v5.11.29)

> 최신 업데이트: GLAPS 운송경로 매칭에서 글로비스 화주 표시값이 `KR10` 화주사코드로 연결되도록 보강했다.
...
- GLAPS 요약 카드는 운송경로 상태/원본시트로 이동하는 필터 버튼이며, `검수메모`는 매칭 키가 아니라 출처·용도·기본값 확인용 메모다. `특이적용건`은 화주사코드+경유지(GLAPS/ELS) 조건 컨샤이니 우선규칙이며, 경유지 빈값은 해당 화주사 기본값이다.
...
- **v5.14.284**: 구간단가 금액표 입력 필터에 `하나라도 포함/모두 포함` 선택을 추가했다. `780000,복화`처럼 금액과 구분을 함께 좁힐 때는 `모두 포함`을 사용하며, 구간단가 첫 진입 기본 범위는 `전체`로 변경했다.
## VERIFICATION
