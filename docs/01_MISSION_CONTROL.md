# ELS MISSION CONTROL (v5.14.328 / APK v5.11.29)

> 최신 업데이트: GLAPS 청구 기준 출발지를 위한 `상차지(청구)`를 운송경로/상세배차/배차변동에 추가하고 N084 현대제철은 KRBNP로 우선 보정한다.
...
- GLAPS 요약 카드는 운송경로 상태/원본시트로 이동하는 필터 버튼이며, 기존 `기타` 항목매핑은 검수메모가 수출입/선사/실출하지/포트/규격/운송사/컨샤이니를 말하면 해당 항목으로 승격한다. `특이적용건`은 적용항목을 `컨샤이니/화주사코드/상차지/상차지(청구)`로 나눠 우선 적용한다. `N084 + 현대제철`은 물리 상차지를 부산신항으로, 청구 상차지를 `KRBNP`로 기본 보정한다. 운송경로 연결키는 `화주사코드 + 상차지(청구)코드 + 경유지코드 + 하차지코드` 기준으로 표시한다.
...
- 상세배차/배차변동은 `상차지` 오른쪽에 `상차지(청구)`를 표시한다. 비어 있으면 물리 상차지로 채우고, 값이 있으면 GLAPS `반출지(출발)코드`와 운송경로 매칭에 우선 사용한다. 이 칼럼은 배차확정/확인완료 후에도 수정 가능하다.
...
## RECENT CHANGES
- **v5.14.328**: GLAPS 운송경로와 상세/변동 테이블에 `상차지(청구)`를 추가했다. 청구 상차지는 물리 상차지와 별도로 저장/수정되며, 운송경로 매칭과 GLAPS 업로드 `반출지(출발)코드`에 우선 적용된다.
