# ELS MISSION CONTROL (v5.14.36 / APK v5.11.17)

> 최신 업데이트: 아산 월간실적 트리/세분화 패널 외곽 폭을 정리했습니다.
...
- **이번 변경 핵심**:
  ...
  - 아산 월간실적 본문은 인포그래픽/구성분석/월별흐름을 먼저 보여주고, `아산매출보고서` 표는 감지됐을 때만 표시한다.
  - 월간 보고서는 기본 `전체` 범위로 합산 표시하고, 월별/일별 트리와 세분화 분석은 외곽 패널까지 compact 폭으로 맞춘다.
