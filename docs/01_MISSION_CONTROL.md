# ELS MISSION CONTROL (v5.13.71 / APK v5.11.12)

> 최신 업데이트: 아산 배차 모바일 기간 카드와 요일별 주/월 선택 흐름을 정리했습니다.
...
- **이번 변경 핵심**:
  - 모바일에서 일별/주별/월별 기간 카드를 한 줄씩 세로 배치.
  - 요일별 작업지 비중 패널의 주간/월간 누적 줄 자체를 선택 컨트롤로 전환.
  - 전체 탭 주간/월간 선택지는 오늘 이후 사전기입 날짜를 제외.
