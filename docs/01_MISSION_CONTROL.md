# ELS MISSION CONTROL (v5.14.99 / APK v5.11.25)

> 최신 업데이트: 행사일정 클릭 시 상세 일정/내용 모달과 날짜별 전체 일정 목록을 볼 수 있도록 모바일 하단 시트형 UI를 보강했습니다.
...
- [x] v5.14.93-99: 연간실적 요일별 비중/포지션 맵, 관리자 활동 로그 이름 검색, 실적관리 스냅샷 DB/경량화, 행사일정 단독 메뉴/상세 UX
...
- Browser: `/employees/events?debug=true` 단독 페이지와 360x780 모바일 가로 넘침 없음 확인
