# ELS MISSION CONTROL (v5.14.242 / APK v5.11.29)

> 최신 업데이트: 아산 `구간단가` 표의 개별 필터 입력줄을 제거하고 조건 필터를 목록형 선택 UI로 정리했다.
...
- 구간단가 조회 범위는 `전체/연도별/월별`이며, 제목열 클릭 정렬과 목록형 조건 필터를 제공한다.
