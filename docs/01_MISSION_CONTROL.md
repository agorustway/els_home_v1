...
| 항목 | 내용 | 비고 |
| :--- | :--- | :--- |
| **현재 버전** | `v5.0.42` | **DONE: 파이썬 모듈 캐싱 이슈 원천 해결 및 WebDAV 우회** |
| **상태** | 🟢 정상 | `dns_fix.py`를 파이썬 모듈 로드 최상단으로 격상 & 로컬 볼륨 직접 읽기 |

## 🚧 최근 이슈 및 조치 (2026-04-21)
- **[SOLVED] 아산 배차판 자동 업데이트 [Errno -2] 장애**: 
  1) `supabase-py`가 내부적으로 사용하는 `httpx` (anyio 기반) 모듈이, `dns_fix.py` 패치가 적용되기도 전에 `socket.getaddrinfo`의 원본을 로컬 변수로 캐싱하는 현상을 발견했습니다. 이를 완벽하게 해결하기 위해 `app_core.py`, `app.py`, `app_bot.py` 등 모든 시작점의 **라인 1번 (심지어 `os`, `sys` 보다 앞)**으로 패치 주입을 끌어올려 DNS 장애를 원천 돌파했습니다 (`v5.0.42`).
  2) 기존 `app_core.py`에 남아있던 구형 WebDAV 다운로드 로직(requests.get)이 DNS 호출을 시도하여 발생하는 오류를 제거하고자, 도커에 마운트된 **로컬 경로(`/app/data`)를 파이썬 내장 Path로 직접 읽도록** 최신 버전과 코드를 동기화했습니다.
