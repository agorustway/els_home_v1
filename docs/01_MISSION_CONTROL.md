# ELS MISSION CONTROL (v5.14.42 / APK v5.11.17)

> 최신 업데이트: 아산 월간실적 범위 전환 데이터를 전면 정규화하고 내부 오류 경계를 추가했습니다.
...
- **이번 변경 핵심**:
  ...
  - 아산 월간실적 본문은 분석 기준(전체/월별/일별), 월 라벨이 보이는 카드형 추세 그래프, 도넛 KPI, 구성/세분화/트리 카드 순서로 표시하며 범위 전환 데이터는 객체 배열로 정규화한다.
