# ELS MISSION CONTROL (v5.14.362 / APK v5.11.29)

> 최신 업데이트: 아산 배차 현황판 예측 손익 API를 필요한 날짜만 조회하도록 경량화했다.
...
- 배차 `현황판`은 첫 화면에서 뷰어 payload를 먼저 보고, 없거나 날짜가 다르면 기존 cache로 fallback한다. 예측 손익은 별도 `/dispatch/forecast` 조회에서 meta로 기간을 잡고 필요한 날짜만 date 조회해 최신 월간 `구간단가`와 매칭한다. 상차지 미확인은 부곡(의왕) 후보로 보정하고, 그래도 안 잡히면 TYPE/구분 평균단가 또는 점검 건수로 표시한다.
