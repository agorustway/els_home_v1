# ELS MISSION CONTROL (v5.14.94 / APK v5.11.25)

> 최신 업데이트: 관리자 활동 로그 검색을 이름과 이메일 모두 조회하도록 확장했습니다.
...
- [x] v5.14.93-94: 연간실적 요일별 비중 다이어그램, 관리자 활동 로그 이름 검색
...
- **v5.14.93**: 연간실적 `요일` 분석을 절대 막대 카드에서 비중형 다이어그램으로 바꿨다. 상단에는 매출 비중·손익 기여·건수 비중 누적 레일을 두고, 요일별 카드는 전체 매출 중 비중, 손익 기여, 건수 비중, 실제 금액/건수를 함께 표시해 어느 요일이 집중되는지 한눈에 보게 했다.
