...
- 운송내역은 상위 `배차판` 옆 탭이며 `/아산지점/2026_수출리스트.xlsx` 월별 시트를 `branch_transport_history`에 저장한다. 설정 모달은 `/아산지점` 기준 NAS 파일 찾기를 제공하고, `출차시간` 헤더는 `청구금액`으로 정규화하며, 동기화는 현재월 1순위 후 인접월/나머지 월 순서로 진행한다.
...
## RECENT CHANGES
- **v5.14.337**: 아산 운송내역 NAS 파일 기본 경로를 `/아산지점/2026_수출리스트.xlsx`로 보정하고, 설정 모달에 파일 `찾기` 버튼과 Supabase `transport_history_path` 컬럼 미반영 시 저장 fallback을 추가했다.
