# ELS MISSION CONTROL (v5.14.308 / APK v5.11.29)

> 최신 업데이트: 안전운임 고시해석(위키트리)을 좌측 트리·우측 문서 구조로 재구성해 1차 고시, 1차 운영지침, 변경건, 2차 예정 가지를 선택 조회한다.
...
- 상세배차 후미 최종 컬럼: `오더구분코드`, `화주사코드`, `반출지(출발)코드`, `작업지(하차지)코드`, `반입지(도착)코드`, `운송서비스코드`, `운송사코드`, `컨샤이니`, `수정일시`. `작업지(하차지)코드`는 GLAPS 원장 코드 헤더만 사용하고 경유지명/ELS명은 표시하지 않는다.
...
## RECENT CHANGES
- **v5.14.308**: 안전운임 고시해석(위키트리)을 실제 좌측 트리/우측 문서형 화면으로 바꿨다. 트리는 `1차 고시 -> 제1~7장`, `1차 운영지침`, `1차-1차 운영지침 변경건`, `2차 고시(아직 안나옴)` 순서로 구성하고, 1차 고시 선택 시 관련 법령·고시 안내를 상세 문서로 보여준다.
