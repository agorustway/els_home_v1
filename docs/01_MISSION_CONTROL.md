# ELS MISSION CONTROL (v5.14.119 / APK v5.11.25)

> 최신 업데이트: 아산 배차판 WEB 입력값이 통합현황/글로비스/모비스 어디서 입력해도 같은 행에 보이도록 익명 TYPE 헤더 복구 기준을 공통화했습니다.
...
- [x] v5.14.64-119: 월간/연간/종합실적 분석, 행사일정, 선적 job, 배차판 DB 누적·자동갱신·WEB 전용 셀 저장 구조 보정, 누적 날짜 탐색 UI 보정
