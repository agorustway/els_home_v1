# ELS MISSION CONTROL (v5.14.342 / APK v5.11.29)

> 최신 업데이트: 아산 운송내역을 선적관리형 UX로 보강해 숨김 드래그/프리셋/일자 필터/컨테이너 이력 조회를 같은 테이블 관리 흐름으로 연결했다.
...
- GLAPS 수식완성본은 원본 `GLAPS컨테이너배차관리` 시트를 그대로 복사하지 않고, 통합셀 값을 행별로 펼친 일반 표로 다시 만든다. 새 날짜 자료는 원본 파일을 바꿔 생성 스크립트를 다시 실행하고, 산출물 안에서 행삭제/선택하여붙여넣기로 보정하지 않는다.
...
- **v5.14.339**: GLAPS 수식완성본의 `GLAPS컨테이너배차관리`를 통합셀 없는 평탄화 표로 생성한다. `GLAPS자동계산`은 평탄화 표의 컨테이너 번호 행만 다시 세므로 날짜별 행수가 달라도 수식이 어긋나지 않는다.
- **v5.14.338**: 아산 운송내역에 좌측 `전체` 카드와 상단 연도 선택을 추가하고, 전체 원장은 DB `rows` 모드로 100건씩 조회한다. 전체 표에는 SEQ 재번호, 검색/필터/정렬/숨김/컬럼이동/P1·P2 프리셋을 적용했다.
- **v5.14.337**: 아산 운송내역 NAS 파일 기본 경로를 `/아산지점/2026_수출리스트.xlsx`로 보정하고, 웹이 DB `target_month` 누적 원장을 조회하는 구조를 회귀 테스트로 고정했다.
- **v5.14.336**: 선적관리 서버 정렬 결과에 확정모선 기본 우선노출을 재적용하지 않도록 해 `작업일 ▲` 오름차순에서 확정모선 행이 상단에 고정되는 현상을 막았다.
- **GLAPS 수식완성 보정**: `GLAPS 26년 6월 업로드양식.xlsx` 수기 원본을 서식 기준본으로 사용하고 Excel COM으로 `ELS` 수식/신규 보조시트를 주입한다. `GLAPS정리`, `CKD고객사코드`, 열폭/목록 서식 보존을 위해 ExcelJS 전체 재저장은 금지한다.
- GLAPS 수식완성본의 빈 원본값은 `0`/`01월 00일`이 아니라 빈칸으로 표시한다. 반대로 값은 있으나 `GLAPS정리` 코드표에 없는 `#N/A`는 확인 필요 신호라 숨기지 않는다.
- **v5.14.335**: 아산 `운송내역` 탭과 `branch_transport_history` 원장을 추가했다. NAS `2026_수출리스트.xlsx`는 월 시트 단위로 동기화하고 `출차시간` 헤더는 `청구금액`으로 통합한다.
- **v5.14.334**: `컨테이너배차관리___20260603110551.xlsx` -> `GLAPS 26년 6월 업로드양식.xlsx` 자동기입 산출물을 생성했다. 193행 입력, 오류 문자열 0건, 확인필요 31행/70항목.
- **v5.14.333**: 상세배차와 `GLAPS_업로드` 반출지(출발)코드는 코드 칼럼을 우선 사용하고, 상차지명 fallback을 차단했다.
- **운영보정**: 글로비스KD외/모비스AS `-1.xlsm`은 `calcMode=manual`로 열기 오류를 막고 `Workbook_Open`에서 자동계산/재계산을 켠다. UDF/배열수식 파일에는 `forceFullCalc/fullCalcOnLoad`를 적용하지 않는다.
...
## VERIFICATION
- 운송내역 UX 보강은 `node --test web/tests/asanTransportHistory.test.mjs`, `npm run lint`, `.next` 정리 후 `npm run build`를 통과했다. 로컬 HTTP 스모크는 `/employees/branches/asan?debug=true`와 `mode=meta/rows` 응답 200을 확인했다. Browser 자동화는 `node_repl` 샌드박스 오류로 대체 검증했다.
...
## IN-PROGRESS
- GLAPS 다음 단계: 보정된 원본서식 보존 수식완성본을 Excel에서 열어 복구창 없이 재계산되는지 확인한 뒤 목록값 미매칭(`LYG`, 신규 작업지/ODCY 세부명칭 등)을 `GLAPS정리`/`CKD고객사코드`에 반영할지 확정한다.
