# ELS MISSION CONTROL (v5.13.56 / APK v5.11.12)

> 최신 업데이트: 아산 연간실적 직접 주입 스크립트의 기본 파일 경로를 NAS 실제 경로 `/volume2/아산지점/...` 기준으로 맞췄습니다.
...
- **이번 변경 핵심**:
  ...
  - 연간실적 최초 적재/타임아웃 우회용 직접 주입 스크립트는 NAS `/volume2/아산지점/...` 파일을 1순위로 사용.
...
| Supabase 인증/DB | 정상 | 연간실적 SQL 적용 완료, 직접 주입 스크립트 준비 |
