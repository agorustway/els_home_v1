# ELS MISSION CONTROL (v5.14.286 / APK v5.11.29)

> 최신 업데이트: GLAPS 특이적용건에 경유지(ELS) `현대제철` → 화주사코드 `N084` 보정 규칙을 추가했다.
...
- GLAPS 요약 카드는 운송경로 상태/원본시트로 이동하는 필터 버튼이며, `검수메모`는 매칭 키가 아니라 출처·용도·기본값 확인용 메모다. `특이적용건`은 컨샤이니 우선코드가 있으면 컨샤이니를 먼저 적용하고, 비어 있으면 경유지(GLAPS/ELS) 기준 화주사코드를 보정한다.
