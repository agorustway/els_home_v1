...
- **이번 변경 핵심**:
  ...
  - 선적관리 필터 결과의 컨테이너를 `/api/els/run`으로 조회해 No.1 메인 이력 행을 엑셀 원장 오른쪽 초록색 컬럼으로 강제 표시.
...
- **v5.13.22**: 컨테이너 이력조회에서 조회 전 WebSquare 그리드 컴포넌트/DOM 행/무자료 모달을 더 넓게 초기화하고, 조회 직후 `데이터가 없음` 문구를 즉시 NODATA로 확정하지 않도록 변경. 선적관리 툴바는 검색/액션 2줄 반응형으로 정리하고, 검색 입력은 1초 지연/Enter 즉시 조회로 바꿔 입력 중 화면 리로드를 방지. 조회 이력은 180일 보존으로 조정하고 최신 조회/보존기간 인덱스를 추가. 이력 컬럼은 렌더링 단계에서 엑셀 원장 오른쪽으로 강제 고정하고 숨김 칩은 소형 말줄임 형태로 단순화.
