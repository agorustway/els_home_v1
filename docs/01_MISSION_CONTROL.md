...
> 최신 업데이트: NAS 글로비스KD외 배차 원본 6.5 시트의 업체별 합계 배열수식 깨짐을 복구했다.
...
- 글로비스KD외 배차 원본은 날짜 시트별 업체별 합계 영역 `B734:C...`의 `GetTextQuantityListFast(O5:W728)` 배열수식 메타가 깨질 수 있다. 6.5 시트는 `B734:C739` 배열수식으로 복구했고, 원본 백업은 `.backup-arrayfix-20260604-142941`이다.
...
## RECENT CHANGES
- **운영 보조**: NAS 글로비스KD외 배차 원본 6.5 시트의 업체별 합계 배열수식이 한 칸 수식으로 깨져 합계 0으로 보이던 문제를 복구했다.
