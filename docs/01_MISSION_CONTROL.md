# ELS MISSION CONTROL (v5.14.337 / APK v5.11.29)

> 최신 업데이트: 아산 운송내역 `2026_수출리스트.xlsx` 기본 경로를 `/아산지점` 직하로 보정하고 NAS 파일 찾기/설정 저장 fallback을 추가했다.
...
- 운송내역은 상위 `배차판` 옆 탭이며 `/아산지점/2026_수출리스트.xlsx` 월별 시트를 `branch_transport_history`에 `target_month` 날짜키로 누적 저장한다. 웹은 NAS 파일을 직접 읽지 않고 DB `mode=meta/date`만 조회하며, 설정 모달은 `/아산지점` 기준 NAS 파일 찾기를 제공한다. `출차시간` 헤더는 `청구금액`으로 정규화하고, 동기화는 현재월 1순위 후 인접월/나머지 월 순서로 진행한다.
