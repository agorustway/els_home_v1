# ELS MISSION CONTROL (v5.14.40 / APK v5.11.17)

> 최신 업데이트: 아산 월간실적 그래프를 카드형 추세로 압축하고 분석 범위 선택 오류를 안정화했습니다.
...
- **이번 변경 핵심**:
  ...
  - 아산 월간실적 본문은 분석 기준(전체/월별/일별), 월간 전용 카드형 추세 그래프, 도넛 KPI, 구성/세분화/트리 카드 순서로 표시한다.
