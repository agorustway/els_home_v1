...
- 배차 `현황판`은 `branch_dispatch_dashboard_cache`의 집계 payload를 우선 사용한다. 첫 화면은 선택일/현재·기본 주월만 담은 `scope=initial`을 받고, full 스냅샷은 백그라운드로 교체한다. 추세는 주말/공휴일을 제외하되 현충일처럼 대체휴일 대상이 아닌 고정 휴일은 월요일 대체로 보지 않는다.
