# ELS MISSION CONTROL (v5.14.06 / APK v5.11.12)

> 최신 업데이트: 아산 연간실적 구간 필터에서 breakdown 월별 근거가 아직 없는 항목도 원인을 표시하고 전체기간 기준으로 확인 가능하게 보정했습니다.
...
- **이번 변경 핵심**:
  ...
  - 최근 12/24개월 등 구간 필터에서 breakdown 월별 세부가 없는 항목은 `월별 근거 갱신 필요`를 표시하고 전체기간 기준 리스트를 fallback으로 노출.
