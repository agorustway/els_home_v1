# ELS MISSION CONTROL (v5.13.53 / APK v5.11.12)

> 최신 업데이트: 아산지점 배차/선적/실적관리 로딩 문구와 폰트 기준을 통일합니다.
...
- **이번 변경 핵심**:
  - 아산지점 초기 로딩 문구를 `데이터를 불러오는 중입니다...`로 통일.
  - 배차/선적/연간실적 로딩 폰트 기준을 `0.86rem / 800 / #64748b`로 통일.
  - 선적관리 필터 재조회 중 빈 테이블 안내도 같은 문구로 정리.

## ACTIVE SYSTEMS
| 영역 | 상태 | 메모 |
|---|---|---|
| Next.js 웹 | 정상 | 아산지점 로딩 문구/폰트 통일 테스트 통과 |
