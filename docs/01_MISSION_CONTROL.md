# ELS MISSION CONTROL (v5.14.293 / APK v5.11.29)

> 최신 업데이트: 상세배차 검수 배지 문구를 압축하고 배차변동 확인완료 행 수정/변경셀 표시를 보강했다.
...
- 상세배차/배차변동 표 헤더는 목록형 다중 선택 필터를 제공한다. `전체선택`은 전체 값 선택, 전체선택 상태에서는 `전체취소`로 동작한다. 배차변동 확인완료 행도 상차지/포트/DG/RF/BKG 선택을 수정할 수 있고 변경 셀은 노란색으로 표시한다.
...
## RECENT CHANGES
- **v5.14.293**: 상세배차 검수 배지를 짧은 코드명으로 압축하고, 배차변동 확인완료 행도 수정 가능하게 풀었다. 추가/삭제/변경 이벤트의 변동 데이터 셀은 노란색으로 표시하며 작업지(하차지)코드는 route 경유지 fallback으로 보강했다.
- **v5.14.292**: GLAPS 항목매핑 `기타` 행을 검수메모 기준으로 `수출입코드/선사/실출하지코드/포트/컨테이너규격/운송사/컨샤이니`에 승격하고, `선사코드, 실출하지코드` 오분류 행은 삭제 대상 처리했다. 중복검출/병합은 `매핑항목+최종코드(BP)` 기준으로 변경했다.
- **v5.14.291**: 상세배차/배차변동 헤더필터 전체선택/전체취소 토글을 추가하고, 특이사항(Nomi,구간)에 `DG`가 있으면 `DG=Y`로 자동 표시한다.
- **v5.14.290**: 상세배차/배차변동 `DG`(위험물)와 `RF`(냉동)를 분리하고, GLAPS 업로드 `위험물 여부/냉동 여부`와 `배차요청시간` 텍스트 출력을 보정했다. 운송경로 매칭은 항목매핑 별칭 코드도 후보로 사용한다.
## VERIFICATION
- 글로비스 원본 `.xlsm`/Supabase 검증: `5.29` 시트와 `branch_dispatch` 2026-05-29 글로비스 `PLMAA/인천` 1행이 `성도(TCR)/CG`로 일치하고 잔여 `TCR` 0건.
- Supabase Advisor/RLS/권한 검증과 운영 웹 스모크는 최근 보안 항목 기준 통과. GLAPS 특이적용건 `waypoint_els_name` 운영 DB 마이그레이션 적용 완료. 남은 WARN은 Auth leaked password Dashboard 설정 1건.
- 최근 코드 변경 검증 내역은 `docs/02_DEVELOPMENT_LOG.md` 각 항목 참조.

## IN-PROGRESS
- GLAPS 다음 단계: Supabase 커넥터 재인증 후 `20260530_glaps_alias_review_note_promotion.sql` 운영 DB 적용, 실제 GLAPS 업로드 샘플 검증.
