# ELS MISSION CONTROL (v5.14.315 / APK v5.11.29)

> 최신 업데이트: 배차변동은 저장 시점별 순변동 기준으로 업체/시간/포트/타입 변경을 변경 셀로 표시하고, 선사 변경은 추가/삭제로 분리한다.
...
- 상세배차/배차변동 다운로드는 기존 우리 기준 시트와 별도로 `GLAPS_업로드` 시트를 추가한다. GLAPS 시트는 상세 1행=1대 기준이며 같은 부킹도 묶지 않고 `컨테이너 수량=1`로 내보낸다. 우리 기준 `시간`과 GLAPS `배차요청일자/배차요청시간`은 텍스트로 출력하고, `POD`는 공란, `선적지`는 우리쪽 업체명으로 출력한다.
...
- 배차변동 확인완료 시 해당 변동의 최종값을 확정 스냅샷 기준으로 반영한다. 이후 재저장은 직전 확인완료 상태 대비 새 변동만 계산한다.
...
- **v5.14.314**: 상세배차 화면에서 `변경건`으로 표시되는 행을 다운로드할 때, 해당 변동 이벤트의 발생/수정 시각을 `수정일시` 열에 기입한다.
- **v5.14.313**: 상세배차/배차변동 화면 다운로드의 `시간/배차요청시간` 열을 텍스트 셀로 저장한다. `GLAPS_업로드`는 `배차요청일자=작업일자`, `POD=공란`, `선적지=업체명`으로 출력한다.
- **v5.14.312**: WEB BKG1~3 단독 변경은 배차변동 추가/삭제 이벤트에서 제외하고, 상세/변동 표의 해당 BKG 셀만 노란색으로 강조한다. 변동 행의 자동 계산 저장 버튼은 `현재값저장`으로 바꿨다.
## VERIFICATION
