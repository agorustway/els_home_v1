# ELS MISSION CONTROL (v5.14.352 / APK v5.11.29)

> 최신 업데이트: 아산 배차 RAG가 특정 운송사 자체/만 질문을 행 단위가 아닌 업체 항목 단위로 집계하도록 보강했다.
...
- 배차 `현황판`은 `branch_dispatch_dashboard_cache`의 집계 payload를 우선 사용한다. 캐시 조회 때 원장 meta 서명이 다르면 서버가 full 원장으로 캐시를 즉시 재생성한다. NAS 프리워밍 인증은 `ASAN_DISPATCH_DASHBOARD_CACHE_TOKEN`과 `SUPABASE_SERVICE_ROLE_KEY` 둘 다 허용한다.
