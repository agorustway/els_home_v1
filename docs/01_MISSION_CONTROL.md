# ELS MISSION CONTROL v4.3.18
> 마지막 업데이트: 2026-03-31 06:28 (KST)

## 📦 최신 배포 정보 (Release)
- **현재 버전**: `v4.3.18` (Build 317)
- **최근 업데이트**: 2026-03-31
- **상태**: 🟢 배포 완료 — 서비스가 꺼진 상태에서 앱 종료 시 알림바가 새로 뜨는 '고스트 알림' 현상 해결
- **APK 다운로드**: [els_driver.apk](https://www.nollae.com/apk/els_driver.apk)
- **설정 파일**: `web/public/apk/version.json` (v4.3.18)

### 🎯 주요 마일스톤
- [x] **v4.3.18**: [HOTFIX] 서비스가 꺼진 상태에서 종료 시 `stopService` 직결로 알림바 오작동(고스트 알림) 완전 해결
- [x] **v4.3.17**: [FIX] 앱 종료 시 오버레이 서비스/알림바 메시지 잔상 버그 수정, [UI] 설정 화면 상단 여백 추가
