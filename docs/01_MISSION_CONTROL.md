# ELS MISSION CONTROL (v5.14.306 / APK v5.11.29)

> 최신 업데이트: 안전운임 변경 추적 위키를 추가해 2026-04-01 운영지침의 변경/해석 포인트를 원문 주석으로 추적한다.
...
## RECENT CHANGES
- **v5.14.306**: 안전운임 변경 추적 위키를 추가했다. 제22호 할증 합산, 인천·평택 기점 할증, 150%·200% 문구 오해, 공휴일·심야 부분 적용 등을 원문 페이지/조문 주석과 트리 검색으로 추적한다.
...
- **v5.14.303**: `데이터 운영 관리`를 인트라넷 관리 메뉴에 추가하고, Header의 인트라넷 드롭다운을 `constants/intranetMenu.js`에서 생성하도록 바꿔 헤더/사이드바/검색 메뉴 누락을 방지했다.
