# ELS MISSION CONTROL (v5.14.283 / APK v5.11.29)

> 최신 업데이트: 상세배차 상차지/포트코드 선택값을 DB 보정값으로 저장하고 배차확정 후에도 수정 가능하게 했다.
...
- **v5.14.282**: GLAPS 특이적용건에 `경유지(ELS)`를 추가했다. `B000034432`의 GLAPS명 `모비스 AS아산센터/AS천안수출물류센터/천안친환경물류센터`와 ELS명 `모비스아산수출물류센터/모비스천안(입장)수출물류센터/모비스천안친환경물류센터` 모두 `GA1588` 우선 적용한다.
