# ELS MISSION CONTROL (v5.14.339 / APK v5.11.29)

> 최신 업데이트: GLAPS 수식완성본의 `GLAPS컨테이너배차관리` 시트를 통합셀 없는 평탄화 표로 생성해 날짜별 행수/붙여넣기 문제를 제거했다.
...
- 운송내역 `전체` 카드는 월 카드 왼쪽에 고정한다. `전체`는 선택 연도 DB 누적 원장을 날짜/입력순으로 합쳐 SEQ를 다시 부여하고, 첫 호출은 100건만 가져온 뒤 `더보기`로 이어 받는다. 연도 선택은 현재/다음 연도와 DB에 존재하는 연도를 함께 표시한다.
