# ELS MISSION CONTROL v3.8.0
> 마지막 업데이트: 2026-03-23 22:00

## 🎯 현재 목표
드라이버 앱 GPS 가시성 개선 및 백그라운드 센서 연동 (v3.7.1)

## 📋 최근 변경 (v3.8.0)
- **오버레이 기능 폐쇄**: APK 배포 시 제한된 설정 허용 문제로 인해 오버레이(Floating Widget)를 완전히 제거하고 PIP 모드에 집중.
- **백그라운드 GPS 최적화**: 오버레이는 사라졌으나, 운행 시작 시 **포그라운드 서비스(Foreground Service)**는 실행되도록 하여 화면이 꺼진 상태에서도 GPS 수집 안정성 확보.
- **권한 목록 정리**: 온보딩 및 설정 화면에서 '오버레이' 권한 항목을 제거하고 가이드 모달 삭제.
- **최종 APK 배포**: `web/public/apk/els_driver.apk` 경로에 최신 최적화 빌드 배치 완료.
