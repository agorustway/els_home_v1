# ELS MISSION CONTROL (v5.14.290 / APK v5.11.29)

> 최신 업데이트: 상세배차/배차변동 `DG`와 `RF`를 분리하고 GLAPS 업로드 시간은 텍스트 `HH:MM`으로 출력한다.
...
- 상세배차/배차변동은 `타입코드`와 `업체명` 사이에 `DG`(위험물)와 `RF`(냉동)를 분리 표시한다. `DG` 기본값은 N, `RF`는 TYPE에 R이 포함된 RF/RE/RH 계열이면 Y로 표시한다.
