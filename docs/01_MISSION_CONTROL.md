# ELS MISSION CONTROL (v5.14.329 / APK v5.11.29)

> 최신 업데이트: GLAPS 운송경로표에 직접 입력한 `상차지(청구)`가 상세/변동 계산의 운송경로 매칭 후 GLAPS 반출지(출발)코드로 반영되게 보정했다.
...
## RECENT CHANGES
- **v5.14.329**: GLAPS 운송경로표에 입력한 `상차지(청구)`를 물리 상차지 경로 매칭 후 상세/변동과 GLAPS 업로드 반출지 코드에 반영하도록 보정했다.
