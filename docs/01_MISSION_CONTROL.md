# ELS MISSION CONTROL (v5.14.292 / APK v5.11.29)

> 최신 업데이트: GLAPS 항목매핑 기타 행을 검수메모 기준 정식 항목으로 승격하고, 중복 기준을 매핑항목+최종코드로 정리했다.
...
- 상세배차/배차변동 표 헤더는 목록형 다중 선택 필터를 제공한다. `전체선택`은 전체 값 선택, 전체선택 상태에서는 `전체취소`로 동작한다.
...
- **v5.14.289**: 상세배차/배차변동 `DG.RF` 선택 칸을 추가하고, TYPE에 R이 있으면 기본 Y로 산출하며 GLAPS 업로드 `냉동 여부`에 반영한다.
## VERIFICATION
