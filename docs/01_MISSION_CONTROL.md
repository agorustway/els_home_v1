# ELS MISSION CONTROL (v5.14.284 / APK v5.11.29)

> 최신 업데이트: 구간단가 필터에 `하나라도 포함/모두 포함` 선택을 추가하고 첫 조회 범위를 `전체`로 바꿨다.
...
- **v5.14.283**: 상세배차 상차지 선택값을 DB override로 저장하고, 배차확정 후에도 상차지/포트코드 선택 변경을 허용했다. 확정 이후 변경된 셀은 붉은 배경으로 표시한다.
## VERIFICATION
