# ELS MISSION CONTROL (v5.13.70 / APK v5.11.12)

> 최신 업데이트: 아산 배차 점유율 카드에 상차지별 비율을 추가하고 톱1점유 표기를 통일했습니다.
...
- **이번 변경 핵심**:
  - 화주 점유율 오른쪽에 `상차지별 비율` 도넛 카드를 추가.
  - 도넛 중앙 보조 라벨과 기간 카드 칩의 `TOP1` 표기를 한글 `톱1점유`로 통일.
  - 수출입/컨테이너 TYPE 카드는 같은 행의 압축 지표 카드로 유지.
