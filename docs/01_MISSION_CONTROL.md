...
> 최신 업데이트: GLAPS 수식완성본이 같은 폴더 최신 `컨테이너배차관리___*.xlsx` 파일을 외부참조하고, 내부 원본 시트를 만들지 않도록 정리했다.
...
- GLAPS 수식완성본은 내부 `GLAPS컨테이너배차관리` 시트를 만들지 않고, 같은 폴더의 최신 `컨테이너배차관리___*.xlsx`를 외부참조한다. 새 다운로드 파일이 생기면 생성 스크립트를 다시 실행해 최신 timestamp 파일로 연결한다.
...
## RECENT CHANGES
- **v5.14.343**: GLAPS 수식완성본은 같은 폴더 최신 `컨테이너배차관리___*.xlsx`를 외부참조하고 내부 원본 시트 복사를 중단했다. `GLAPS자동계산`은 외부 파일 X열 컨테이너 행만 200건 슬롯으로 산출한다.
