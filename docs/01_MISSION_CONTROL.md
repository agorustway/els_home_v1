# ELS MISSION CONTROL (v5.14.124 / APK v5.11.25)

> 최신 업데이트: Vercel 배포 시 `api/els/*` 함수가 로컬 elsbot 대형 파일을 번들링하지 않도록 trace exclude를 추가했습니다.
...
- [x] v5.14.64-124: 월간/연간/종합실적 분석, 행사일정/공휴일, 선적 job, 배차판 DB 누적·자동갱신·WEB 전용 셀 저장 구조 보정, 누적 날짜 탐색 UI 보정
