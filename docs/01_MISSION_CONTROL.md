# ELS MISSION CONTROL (v5.14.41 / APK v5.11.17)

> 최신 업데이트: 아산 월간실적 월별/일별 선택 런타임 오류와 그래프 월 라벨을 보정했습니다.
...
- **이번 변경 핵심**:
  ...
  - 아산 월간실적 본문은 분석 기준(전체/월별/일별), 월 라벨이 보이는 카드형 추세 그래프, 도넛 KPI, 구성/세분화/트리 카드 순서로 표시한다.
