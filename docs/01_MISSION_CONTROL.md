# ELS MISSION CONTROL (v5.13.78 / APK v5.11.12)

> 최신 업데이트: 아산 연간실적 날짜 시리얼·시간 문자열·금액 표시를 importer와 화면에서 정규화했습니다.
...
- **이번 변경 핵심**:
  - 연간실적 `마감월`은 `YYYY-MM`, `작업일자`는 `YYYY-MM-DD`로 정규화.
  - Excel 날짜 시리얼(`42006`)과 ISO 시간 문자열을 날짜 표시값으로 변환.
  - `청구`/`하불` 등 금액 컬럼은 천단위 구분 표시.
  - 배차 검색/필터 합계와 실행사 지역칸 파싱 보강 유지.

## ACTIVE SYSTEMS
| 영역 | 상태 | 메모 |
|---|---|---|
| Next.js 웹 | 정상 | 아산 배차 분석 대시보드/선적/실적 테스트 통과 |
| Supabase 인증/DB | 정상 | 연간실적 직접 주입 완료, 날짜/금액 표시 정규화 반영 필요 |
