# ELS MISSION CONTROL (v5.14.183 / APK v5.11.29)

> 최신 업데이트: 아산 배차변동내역도 상세배차와 같은 GLAPS 계산/입력 구조로 표시하고, 상세배차의 active 변동건은 하단으로 모아 표시한다.
...
- 변동행은 상세배차와 같은 컬럼 계산을 적용한다. 상차지/BKG/기본 입력값을 수정하면 운송경로, 포트/라인/타입, GLAPS 최종코드를 다시 도출해 저장한다.
...
## RECENT CHANGES
- **v5.14.183**: 배차변동내역 행을 상세배차 line으로 재구성해 GLAPS 코드 계산을 동일하게 적용한다. 변동행의 상차지/BKG/기본 입력값은 직접 수정 가능하며 저장 시 계산된 rowValues와 rowContext를 함께 반영한다. 상세배차 화면의 active 변동건은 목록 하단으로 보내고 마지막 칸에 `변경건` 배지를 표시한다.
