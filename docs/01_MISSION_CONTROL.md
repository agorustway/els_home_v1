# ELS MISSION CONTROL v3.7.3
> 마지막 업데이트: 2026-03-23 21:35

## 🎯 현재 목표
드라이버 앱 GPS 가시성 개선 및 백그라운드 센서 연동 (v3.7.1)

## 📋 최근 변경 (v3.7.3)
- **권한 안내 UI 롤백**: 사용자의 요청에 따라 v3.7.1의 심플한 리스트 스타일로 복구.
- **PIP 실시간 동기화 수정**: PIP 모드 진입 시 메인 화면의 최신 GPS 상태(`1분(서행)` 등)가 즉시 반영되도록 딜레이 제거.
- **APK 배포**: 롤백된 UI와 동기화 수정 사항이 포함된 `els_driver.apk` 빌드 완료.
