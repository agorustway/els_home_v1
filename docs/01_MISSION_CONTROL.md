# ELS MISSION CONTROL (v5.14.324 / APK v5.11.29)

> 최신 업데이트: 아산 배차 현황판은 meta-only 날짜가 남아 있으면 전체 집계 원장을 보강 조회해 주간/월간 카드가 빈 값으로 보이지 않게 했다.
...
| 영역 | 상태 | 메모 |
|---|---|---|
| Next.js 웹 | 정상 | 배차판 표는 경량 meta/date 조회, 현황판은 집계 원장 보강 조회 |
...
- 배차 `현황판`은 주간/월간/요일별 집계를 위해 meta-only 날짜가 남아 있으면 full 원장을 보강 조회한다. 배차판/상세/변동 표는 선택 날짜 중심 지연 조회를 유지한다.
