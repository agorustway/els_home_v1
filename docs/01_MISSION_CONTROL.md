# ELS MISSION CONTROL (v5.13.5 / APK v5.11.12)

> 최신 업데이트: 앱 로컬 GPS 안정화/중복 전송 방어와 운행 종료 후 Android 서비스 잔류 방지를 적용했습니다.
...
- **이번 변경 핵심**:
  - 지도 화면은 1초 수신을 유지하되 앱 로컬 안정화 통과 포인트만 마커/경로/서버 전송에 반영.
  - 정차·저속·실내 흔들림은 후보로 보류하고, 같은 위치/방향으로 연속 확인될 때만 확정점으로 승격.
  - 일반 운행은 배터리 보호형 수집/전송으로 낮추고, 웹 실시간 추적·앱 지도 화면만 고감도 모드로 운영.
  - 서버는 중복 위치 저장 전 geocode를 생략하고, 이동거리/heartbeat 기준 미달 포인트를 저장하지 않음.
  - 운행 종료 후 뒤로가기 종료 시 Android 오버레이/포그라운드 서비스가 재기동되지 않도록 차단.
