# ELS MISSION CONTROL (v5.14.109 / APK v5.11.25)

> 최신 업데이트: 종합실적 제목과 주요 경영 지표 문구를 컨테이너 운송 통합실적/이익률/자사 비율 기준으로 정리했습니다.
...
- **이번 변경 핵심**:
  ...
  - 종합실적 제목은 `컨테이너 운송 통합실적`, 주요 비율 문구는 `이익률`/`자사 비율`로 통일한다. 계약/차량 집중도는 매출/건수 비중을 구분하고, 연간·월간실적 테이블 검색은 금액 쉼표·공백을 정규화한다.
...
- [x] v5.14.93-109: 연간실적 요일별 비중/포지션 맵, 관리자 로그, 실적관리 스냅샷/용어, 행사일정, 선적 조회 job, 실적 설정/근거표/차량 기준 정리, 배차판/선적 DB 방어 보정
