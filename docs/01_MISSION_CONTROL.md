...
| 항목 | 내용 | 비고 |
| :--- | :--- | :--- |
| **현재 버전** | `v5.0.44` | **DONE: Bytes vs String 타입 호환 이슈 해결 완료** |
| **상태** | 🟢 정상 | `httpx` 내부 인코딩 바이패스 방어 완료 |

## 🚧 최근 이슈 및 조치 (2026-04-21)
- **[SOLVED] 아산 배차판 자동 업데이트 [Errno -2] 끈질긴 장애 완전 종결**: 
  1) `httpx` (내부 `httpcore`) 모듈이 URL의 호스트네임을 파이썬 표준인 문자열(`str`)이 아니라 바이트열(`bytes`, 예: `b"pzfnrnsc..."`) 형태로 파이썬 내장 네트워크 함수(socket)에 전달하는 충격적인 사실을 적발했습니다. 
  2) 기존 `dns_fix.py`의 몽키패치는 호스트네임이 문자열(`str`)일 때만 IP를 반환하도록 작성되어 있어, 바이트열로 들어오는 `httpx`의 요청은 매핑 사전을 통과하지 못하고 그대로 무시(Bypass)되어 OS의 DNS에 요청을 보내 오류를 뱉고 있었습니다.
  3) 이를 해결하기 위해 DNS 패치 함수와 소켓 래퍼 모두에 들어오는 인자를 `utf-8` 문자열로 강제 변환(`decode`)하여 검사하도록 로직을 수정했습니다 (`v5.0.44`). 이제 네트워크 속의 그 어떤 함정도 우리의 DNS 하이재킹을 피해 갈 수 없습니다.
