...
| 항목 | 내용 | 비고 |
| :--- | :--- | :--- |
| **현재 버전** | `v5.0.45` | **DONE: HTTPCore 최하단 종속성 직접 하이재킹 완료** |
| **상태** | 🟢 정상 | `socket` 모듈 우회 대비, `httpcore._backends` 직접 래핑 추가 |

## 🚧 최근 이슈 및 조치 (2026-04-21)
- **[SOLVED] 아산 배차판 자동 업데이트 [Errno -2] 끈질긴 장애 완전 종결**: 
  1) `httpx` (내부 `httpcore`) 모듈이 우리가 걸어둔 파이썬 표준 `socket` 패치를 교묘한 방식으로 우회(또는 C 레벨 에러 선점)하여 여전히 `gaierror`를 내뿜고 있는 독자적인 결함을 최종 파악했습니다.
  2) 기존 `socket.getaddrinfo`, `socket.socket`을 하이재킹하는 것에 더해, 아예 **에러 발생의 진원지인 `httpcore._backends.sync.SyncBackend.connect_tcp` 함수 자체를 몽키패치**해버리는 "삼중 그물망(Triple Net)"을 구성했습니다 (`v5.0.45`).
  3) 이제 `httpcore`가 파이썬 내장 라이브러리를 호출하기 *전에* 우리 코드가 먼저 가로채서 호스트네임을 IP로 강제 변환합니다. 모든 네트워크 요청은 완전히 통제됩니다.
