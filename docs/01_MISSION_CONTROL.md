...
| 항목 | 내용 | 비고 |
| :--- | :--- | :--- |
| **현재 버전** | `v5.0.43` | **DONE: C 확장 모듈의 DNS 우회 원천 차단 (소켓 객체 래핑)** |
| **상태** | 🟢 정상 | `socket.socket` 자체를 래핑(Wrapping)하여 C 레벨 접근 완전 봉쇄 |

## 🚧 최근 이슈 및 조치 (2026-04-21)
- **[SOLVED] 아산 배차판 자동 업데이트 [Errno -2] 끈질긴 장애 완전 종결**: 
  1) `httpx` (내부 `httpcore`) 모듈이 파이썬의 `socket.getaddrinfo`를 자체적으로 호출하는 것이 아니라, C 언어로 구현된 `_socket.socket.connect`를 직접 호출하면서 우리의 몽키패치를 무력화하고 있는 현상을 적발했습니다. 
  2) 이에, 파이썬 클래스 상속을 통해 `original_socket = socket.socket`을 하이재킹하고, `connect` 메서드 자체를 오버라이드한 뒤 C 레벨(OS)로 내려가기 **직전** 호스트명(도메인)을 우리가 강제 지정한 IP(`172.64.149.246`)로 치환해 버리도록 아키텍처를 재설계했습니다 (`v5.0.43`).
  3) 이 방식은 TLS 핸드셰이크의 SNI(Server Name Indication) 무결성을 전혀 해치지 않으면서도 (Cloudflare 호환) 무작정 IP로 통신하게 만드는 궁극적인 DNS 해결책입니다.
