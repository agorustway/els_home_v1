# ELS MISSION CONTROL (v5.14.184 / APK v5.11.29)

> 최신 업데이트: 아산 배차변동내역의 직접 입력을 제한하고, 상세배차와 같은 GLAPS 계산 표시/확인 중심 구조로 정리했다.
...
- 변동행은 상세배차와 같은 컬럼 계산을 적용하되 전방위 직접 입력은 금지한다. 원천 수정은 WEB 부킹/비고 또는 엑셀 배차 원본에서 처리하고, 변동내역은 계산값 반영/확인 이력만 남긴다.
