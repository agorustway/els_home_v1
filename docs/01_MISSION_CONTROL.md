# ELS MISSION CONTROL (v5.14.289 / APK v5.11.29)

> 최신 업데이트: 상세배차/배차변동에 `DG.RF` Y/N 칸을 추가하고 RF 타입은 기본 Y로 표시한다.
...
- 구간단가 TYPE처럼 영문이 포함된 검색어는 금액 숫자 보조검색을 적용하지 않아 `20RF`가 기간 `2026`의 `20`에 과매칭되지 않게 한다. 입력 필터는 쉼표/세미콜론 조건을 `하나라도 포함` 또는 `모두 포함`으로 선택한다.
...
- GLAPS 요약 카드는 운송경로 상태/원본시트로 이동하는 필터 버튼이며, `검수메모`는 매칭 키가 아니라 출처·용도·기본값 확인용 메모다. `특이적용건`은 컨샤이니 우선코드가 있으면 컨샤이니를 먼저 적용하고, 비어 있으면 경유지(GLAPS/ELS) 또는 상세행 화주명 기준으로 화주사코드를 보정한다.
- 상세배차 후미 최종 컬럼: `오더구분코드`, `화주사코드`, `반출지(출발)코드`, `작업지(하차지)코드`, `반입지(도착)코드`, `운송서비스코드`, `운송사코드`, `컨샤이니`, `수정일시`.
- 상세배차/배차변동은 `타입코드`와 `업체명` 사이에 `DG.RF` Y/N 칸을 둔다. 기본값은 N이고, TYPE에 R이 포함된 RF/RE/RH 계열은 Y로 표시한다.
...
- **v5.14.288**: 상세배차 하차지 `광양`을 GLAPS 운송경로 코드 `KRKAN` 후보로 확장해 `KR10-KRKAN-글로비스KD센터3포장-KRKAN` 경로가 도출되게 했다.
- **v5.14.287**: GLAPS 화주사코드 특이적용건이 상세행 `작업지`뿐 아니라 `화주` 값도 매칭 후보로 보게 해, `현대제철` 행이 `N084`로 들어가게 했다.
- **v5.14.286**: GLAPS 특이적용건에 경유지(ELS) `현대제철` → 화주사코드 `N084` 보정 규칙을 추가했다. 컨샤이니 우선코드가 빈 특이적용건은 경유지 기준 화주사코드 보정으로 처리한다.
- **v5.14.285**: GLAPS 운송경로 원장의 `KR10` 화주사코드를 상세배차 화주명 `글로비스/글로비스KD외/현대글로비스`와 연결해, `GLC00035` 등 글로비스 운송경로가 미도출되지 않게 했다.
## VERIFICATION
