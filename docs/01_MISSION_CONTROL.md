# 🚩 ELS MISSION CONTROL (v5.8.0)

> **"Omni-Agent 완성과 지능형 데이터 리포팅 (v5.8.0)"**
> - **최근 업데이트**: NAS 문서 확장자별 통계 및 실시간 날씨/미세먼지 통합 브리핑 도입.
> - **주요 상태**: AI 어시스턴트 Omni-RAG 고도화 및 시스템 안정화 완료.
...
- **이슈**: `els-bot` 컨테이너 메모리 부족 및 IO 잠금 이슈 해결 완료.
- **조치**: 
  1. `docker-compose.yml` `shm_size: 2gb` 적용 및 `/tmp` 프로필 경로 변경으로 크래시 해결.
  2. Omni-Agent NAS 파싱 통계 엔진에 확장자별 집계 로직 추가 반영.
