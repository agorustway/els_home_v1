...
#
# 마지막 업데이트: 2026-04-01
# 근거: Vercel 트래픽 나스 이관 반영 + docs/ 구조 개혁 + AI/IDE Blueprint 도입 + AI 권한/도구 요청 프로토콜 추가
...
## 🧰 제8-1조. AI 작업 권한/도구 요청 프로토콜

> **"막히면 우회하지 말고, 형에게 정확히 요청한다."**

1. **요청 우선**: 네트워크, Git push, NAS/Supabase/Vercel/GitHub 접근, Android/Gradle 빌드, 브라우저 자동화, OS 파일 권한, 추가 CLI 설치가 필요하면 임의 우회하지 말고 형에게 요청한다.
2. **요청 형식**: 필요한 항목, 막힌 이유, 실행할 명령어, 영향 범위, 되돌리는 방법이 있으면 함께 짧게 보고한다.
3. **설치 구분**: 새 도구는 필수/선택을 구분해 제안한다. 현재 작업에 꼭 필요하지 않은 도구는 설치를 강요하지 않는다.
4. **반복 이슈 기록**: 같은 권한/도구 문제가 반복되면 `docs/08_ENVIRONMENT_SETUP.md`에 해결 명령과 주의사항을 남겨 다음 세션이 바로 요청할 수 있게 한다.
...
---
*최종 갱신일: 2026-05-16 (by Codex — AI 권한/도구 요청 프로토콜 추가)*
