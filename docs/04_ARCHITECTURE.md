...
## 6. 🚛 컨테이너 관제 최적화 전략 (GPS Optimization)

스마트폰 배터리 효율과 이동 궤적의 정확도를 동시에 잡기 위해 **속도 기반 동적 전송 주기(Dynamic Interval)**를 적용한다.

### 6-1. 속도별 GPS 수송 주기 가이드
위치 수집은 지속하되, 서버 전송 주기를 속도에 따라 조절한다.

| 운행 상태 | 속도 범위 | 권장 전송 주기 | 기대 효과 |
| :--- | :--- | :--- | :--- |
| **🚀 실시간 추적** | **관리자 요청 시** | **3초 (강제)** | **실시간 고정밀 경로 마킹 및 현장 모니터링** |
| **고속 주행** | 70km/h 이상 | 30초 ~ 1분 | 고속도로 이탈 방지 및 매끄러운 궤적 |
| **일반 주행** | 20 ~ 70km/h | 1분 ~ 2분 | 시내 주행 경로 파악 및 신호 대기 대응 |
| **정체/서행** | 5 ~ 20km/h | 3분 ~ 5분 | 불필요한 데이터 전송 억제 |
| **정차/대기** | 5km/h 미만 | 10분 ~ 15분 | 배터리 절약 모드 진입 (상하차/부두 대기) |

### 6-2. 수집 고도화 기술 (Roadmap)
1. **거리/각도 기반 수집**: 직선 주행 시 주기를 늘리고, 방향(Heading)이 급격히 꺾일 때 즉시 전송하여 곡선 구간 정확도 향상.
2. **스마트폰 센서(가속도계) 활용**: 장시간 움직임이 없는 경우(취침/식사) GPS 센서를 비활성화하고 마지막 위치 유지.
3. **PiP/백그라운드 대응**: `navigator.geolocation.watchPosition`과 `MediaSession API`를 조합하여 앱이 백그라운드에 있을 때도 수집 정지를 방지함.
