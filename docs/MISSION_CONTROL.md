...
- [2026-03-11] **아산지점 모비스AS 데이터 유실 해결**: '국가명' 헤더 정규화('국가') 및 통합현황 매핑 수정으로 데이터 정상 노출 완료.
- [2026-03-09] **자료실 다운로드 로직 일원화 및 호환성 강화**: 업무자료실/서식자료실 UI 통일, NAS 파일 프록시 다운로드 구현, `Content-Disposition` RFC 5987 대응(한글 파일명 깨짐 방지).
- [2026-03-09] **자유게시판 UI/UX 고도화**: 작성자 ID 대신 실명 및 직급 표시(profiles+user_roles 교차 검증), 상세 페이지 전용 디자인(Premium) 적용 및 반응형 레이아웃 보강.
- [2026-03-09] **컨테이너 이력조회 UI/UX 편의 보강**: 로그 패널 기본 펼침 설정 및 서버 의존성 없는 클라이언트 사이드 엑셀 다운로드(XLSX) 재구현.
- [2026-03-09] **안전운임 구간/경로 조회 UI/UX 전면 고도화**: 고시(2026 제정안) 기점/좌표 최적화, 행정복지센터 자동 검색/변환, 3번 줄 연동 UX 개선, 다중 운임 저장(SessionStorage), XLSX 엑셀 열 너비 자동 확장 및 디자인 통일.
