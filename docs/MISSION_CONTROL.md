...
| **모바일 제어** | Media Session API / Wake Lock | `/driver-app` 독립형 |
...
### 🔴 진행 중
- 차량위치관제 시스템 실운영 테스트 및 안정화 (운전원 실제 피드백 수렴)

### 🟡 알려진 이슈
- NAS CPU 부하 시 Selenium 타임아웃 증가 경향 (세션 2개로 제한 및 간격 60초 연장 조치 완료)
- 봇 테스트 빈도 주의 (ETRANS 측 봇 탐지 가능성)
- Media Session API 지원 브라우저 차이 (iOS/Android 최신 브라우저 권장)

### 🟢 최근 완료 (최신 5건)
- [2026-03-19] **Vercel 빌드 오류 해결 및 에러 페이지 분리**: Route Group 구조에서 최상위 레이아웃 부재로 인한 빌드 에러 수정. `(main)`과 `(standalone)` 그룹별 전용 `not-found.js`, `error.js` 구축 및 최상위 파일 제거.
- [2026-03-19] **드라이버 앱 구조적 완전 분리 (Route Groups)**: 인트라넷과 앱을 `(main)`과 `(standalone)` 그룹으로 완전 분리하여 독립 루트 레이아웃 구축. 헤더/메뉴 트리 등 인트라넷 UI 원천 차단 및 전용 PWA 설치성 강화.
- [2026-03-18] **운전원 전용 독립 앱 구축 (`/driver-app`)**: Media Session API 통한 알림창/잠금화면 운행 제어(Pause/Resume/Stop), 무음 오디오 루프 통한 백그라운드 유지, 전용 manifest_driver.json 구성.
