export const AI_ASSISTANT_VERSION = 'v5.14.344';
...
export function getAiAssistantIntroMessage() {
  return [
    ...
    '',
    '현재는 사내 데이터베이스에 누적된 업무 데이터와 웹사이트 등록 자료를 기준으로 답변합니다.',
    '아산 배차판·운송내역·상세배차·변동내역·GLAPS코드·선적관리·실적관리는 운영 데이터베이스와 화면 도출항목을 읽어 답변합니다.',
    '',
    '안전운임, 컨테이너 이력, 차량 관제, 사내 게시글, 업무자료실, 연락처, 작업지, 날씨, 유가, 법령까지 연결해서 확인합니다.',
    '이미지나 NAS 원본 파일을 직접 파싱한다고 말하지 않고, 데이터베이스에 저장되거나 색인된 자료 기준으로 답합니다.',
  ].join('\n');
}
...
export function getAiQuickPrompts() {
  return [
    ...
    { label: '6월 아산 운송내역 KCC 청구금액 찾아줘', icon: null },
...
export function getAiAssistantGuideSections() {
  return [
    {
      title: '아산 배차판 데이터베이스 조회',
      description: '운영 데이터베이스를 기준으로 배차판, 상세배차, 변동내역, GLAPS코드를 함께 읽습니다.',
      examples: [
        '오늘 아산 배차 총 몇 대야?',
    ...
    },
    {
      ...
      examples: [
        ...
        'CMAU7631738 운송내역에서 확인해줘',
      ],
    },
    {
      title: '아산 선적관리 데이터베이스 조회',
      description: '선적관리에 누적된 행과 컨테이너 이력 조회 결과를 검색합니다.',
      examples: [
    ...
    {
      title: '안전운임 및 물류 관제',
      description: '안전운임 공식 데이터, 컨테이너 이력, 차량 운행 데이터베이스를 연결합니다.',
      examples: [
        '아산 부산 40ft 안전운임 얼마야?',
...
export function getAiSystemCapabilitySummary() {
  return [
    '## 현재 ELS AI 실제 연결 범위',
    '- 사내 데이터베이스: 연락처, 게시글/업무보고, 작업지, 업무자료실, 차량 운행/위치, 아산 배차판/운송내역/상세배차/변동내역/GLAPS/선적관리/실적관리.',
