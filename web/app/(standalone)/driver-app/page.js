...
export default function DriverAppPage() {
    ...
    const startGPS = useCallback(async (tripId, status, isRealtime = false) => {
        ...
        if (BackgroundGeolocation) {
            ...
            try {
                const watcherId = await BackgroundGeolocation.addWatcher({
                    backgroundMessage: isRealtime ? "🔴 실시간 고정밀 관제가 진행 중입니다." : "ELS 실시간 운송 관제가 진행 중입니다.",
                    backgroundTitle: isRealtime ? "실시간 관제 활성" : "운행 중",
                    requestPermissions: true,
                    stale: false,
                    distanceFilter: isRealtime ? 0 : 10 
                }, (location) => {
                    const now = Date.now();
            ...
            if (data.trips) {
                ...
                if (active) {
                    ...
                    if (active.status === 'driving') { 
                        startGPS(active.id, 'driving', !!active.realtime_mode); 
                        playSilence(); 
                        ...
                        if (Overlay) {
                            Overlay.showOverlay({ 
                                ...
                                container: active.realtime_mode ? `🔴 실시간 추적 중` : `📦 ${active.container_number || '미입력'}`,
                                tripId: active.id,
        ...
        <motion.div className={styles.tabContent} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <div className={styles.formSection}>
                ...
                <div style={{ color: '#64748b', fontSize: '0.85rem' }}>
                    <p>버전: v3.9.9.6 (4-Tab Recovery)</p>
    ...
    return (
        <div className={styles.driverPage}>
            ...
            <nav className={styles.bottomNav}>
                ...
                <button className={`${styles.navItem} ${activeTab === 'notice' ? styles.navItemActive : ''}`} onClick={() => { triggerHaptic('LIGHT'); setActiveTab('notice'); }}>
                    <span className={styles.navIcon}>📢</span>
                    <span>공지</span>
                </button>
            ...
            <style jsx global>{`
                ...
                .${styles.bottomNav} { grid-template-columns: repeat(4, 1fr) !important; }
