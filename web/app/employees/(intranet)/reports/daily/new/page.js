...
export default function NewDailyReportPage() {
    ...
    useEffect(() => {
        if (!authLoading) {
            if (!role || role === 'visitor') {
                router.replace('/login?next=/employees/reports/daily/new');
            } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                router.push('/employees/reports/daily');
                router.refresh();
            }
        } catch (error) {
            console.error(error);
            ...
            <div className={styles.editorCard}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className={styles.formGroup}>
                        ...
                        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="예: 2025-01-30 일일 업무일지" className={styles.input} required />
                    </div>
                    <div className={styles.formGroup}>
                        <label className={styles.label}>내용</label>
                        <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="당일 업무 내용을 입력하세요" className={styles.textarea} required />
                    </div>
                    <div className={styles.formGroup}>
                    ...
                    <div className={styles.editorActions}>
                        ...
                        <button type="submit" disabled={submitting} className={styles.btnPrimary}>일지 등록</button>
                    </div>
                </form>
            </div>
        </div>
