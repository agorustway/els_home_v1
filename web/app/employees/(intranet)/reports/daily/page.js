...
export default function DailyReportsPage() {
    ...
    useEffect(() => {
        if (!authLoading && !role) {
            router.replace('/login?next=/employees/reports/daily');
        }
    ...
    async function fetchReports() {
        try {
            ...
            const res = await fetch(`/api/board?type=report&kind=daily${branchQuery}`);
    ...
    const branchTitle = ['admin', 'headquarters'].includes(role) ? '일일 업무일지 (전체)' : `${getRoleLabel(role)} 일일 업무일지`;
            ...
            <div className={styles.detailCard}>
                <table className={styles.boardTable}>
                    ...
                    <tbody>
                        ...
                        {posts.length === 0 && (
                            <tr>
                                <td colSpan="5" className={styles.empty}>작성된 일일 업무일지가 없습니다.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
