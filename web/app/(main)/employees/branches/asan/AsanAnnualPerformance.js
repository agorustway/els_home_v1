            ...
            {!item || series.length < 2 ? (
                ...
                <>
                    ...
                    <div className={styles.routeUnitTrendSummary}>
                        ...
                        <div><span>최저 차액단가</span><strong>{formatPerformanceAmount(lowProfit?.unitProfit)}</strong><em>{lowProfit?.label || '-'}</em></div>
                    </div>
                </>
            )}
        </section>
...
function RouteUnitPricePanel({
    ...
    const maxUnitRevenue = Math.max(1, ...groups.map(item => Math.abs(safeNumber(item.unitRevenue))));
    const scopeButtons = [
        { key: 'all', label: '전체' },
        { key: 'year', label: '연도별' },
        { key: 'month', label: '월별' },
    ...
    return (
        <div className={styles.routeUnitShell}>
            ...
            <div className={styles.routeUnitKpis}>
                <div><span>평균 청구단가</span><strong>{formatPerformanceAmount(totals.unitRevenue)}</strong><em>{safeNumber(totals.rowCount).toLocaleString('ko-KR')}건 기준</em></div>
                <div><span>평균 하불단가</span><strong>{formatPerformanceAmount(totals.unitPurchase)}</strong><em>매입 단가</em></div>
                <div><span>평균 차액단가</span><strong className={safeNumber(totals.unitProfit) < 0 ? styles.negative : styles.positive}>{formatPerformanceAmount(totals.unitProfit)}</strong><em>청구단가 - 하불단가</em></div>
                <div><span>단가 산출 구간</span><strong>{groups.length.toLocaleString('ko-KR')}</strong><em>{data?.summary?.sourceFileCount || 0}개 원장 통합</em></div>
            </div>
                        ...
                        <div className={styles.routeUnitRows}>
                            ...
                            {groups.map(item => (
                                ...
                                >
                                    ...
                                    <b>{formatPerformanceAmount(item.unitPurchase)}</b>
                                    <b className={safeNumber(item.unitProfit) < 0 ? styles.negative : styles.positive}>{formatPerformanceAmount(item.unitProfit)}</b>
                                    <em>{safeNumber(item.rowCount).toLocaleString('ko-KR')}</em>
                                    <i><DataBar value={item.unitRevenue} max={maxUnitRevenue} tone="revenue" /></i>
                                </button>
                            ))}
                        </div>
                    )}
                        ...
                        {selected ? (
                            <>
                                ...
                                <div className={styles.routeUnitAmountGrid}>
                                    <div><span>청구단가</span><strong>{formatPerformanceAmount(selected.unitRevenue)}</strong><em>{safeNumber(selected.rowCount).toLocaleString('ko-KR')}건 평균</em></div>
                                    <div><span>하불단가</span><strong>{formatPerformanceAmount(selected.unitPurchase)}</strong><em>건당 매입</em></div>
                                    <div><span>차액단가</span><strong className={safeNumber(selected.unitProfit) < 0 ? styles.negative : styles.positive}>{formatPerformanceAmount(selected.unitProfit)}</strong><em>건당 차액</em></div>
                                </div>
                            </>
                        ) : (
