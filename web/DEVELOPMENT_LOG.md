...
## [2026-02-02] - 아산지점 식단표 삭제 기능 완전 해결 (RLS 우회)

### 🚀 핵심 이슈
- **삭제 미반영**: 삭제 완료 메시지는 뜨지만, 실제로는 DB에서 지워지지 않고 새로고침 시 데이터가 남는 현상 해결.
- **RLS 정책 미비**: `weekly_menus` 테이블에 `DELETE` 정책이 없어 일반 사용자 계정으로는 삭제가 불가능했음.

### 🛠️ 작업 상세
#### 1. API 로직 고도화 (`web/app/api/asan/lunch/route.js`)
- **Admin Client 도입**: DB 레벨의 RLS 정책을 우회하여 확실하게 삭제를 실행할 수 있도록 `createAdminClient`를 적용했습니다.
- **권한 검증 강화**: RLS를 우회하는 만큼, 서버 사이드에서 `user_roles` 테이블을 조회하여 요청자가 `admin` 권한을 가졌는지 엄밀하게 검증하는 로직을 추가했습니다.
- **전체 복구**: 이전 작업 중 코드 누락이 있었던 GET, POST 핸들러를 완벽하게 복구했습니다.
