...
test('안전운임 압축 안내는 원문 주석번호와 변경 추적 위키로 이어진다', () => {
...
test('안전운임 안내 UI는 모바일에서 요약과 포인트가 잘리지 않도록 구성한다', () => {
  ...
  assert.match(modalPage, /employees\/safe-freight\/wiki/, '메인 화면에서 변경 추적 위키로 이동할 수 있어야 합니다.');
  ...
  assert.match(wikiPage, /type="search"/, '변경 추적 위키에는 검색 입력이 있어야 합니다.');
  assert.match(wikiPage, /filterTree/, '변경 추적 위키는 트리 검색 필터를 제공해야 합니다.');
  ...
  assert.match(wikiCss, /\.content\s*\{[\s\S]*grid-template-columns/, '변경 추적 위키는 트리와 주석 패널을 나눠야 합니다.');
  assert.doesNotMatch(wikiCss, /letter-spacing:\s*-/, '변경 추적 위키에 음수 자간이 남으면 안 됩니다.');
});
