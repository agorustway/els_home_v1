...
test('안전운임 압축 안내는 원문 주석번호와 고시해석 위키트리로 이어진다', () => {
  ...
  const firstNotice = SAFE_FREIGHT_WIKI_TREE.find((node) => node.id === 'notice-1st');
  assert.deepEqual(
    rootTitles,
    ['1차 고시', '1차 운영지침', '1차-1차 운영지침 변경건', '2차 고시(아직 안나옴)'],
    '고시해석 위키트리는 1차/운영지침/변경건/2차 예정 순서여야 합니다.'
  );
  assert.equal(firstNotice?.children?.length, 7, '1차 고시는 제1장부터 제7장까지 하위 트리를 가져야 합니다.');
  ...
  assert.match(wikiText, /1차-1차 운영지침 변경건/, '1차와 운영지침의 변경건 비교 가지가 있어야 합니다.');
...
test('안전운임 안내 UI는 모바일에서 요약과 포인트가 잘리지 않도록 구성한다', () => {
  ...
  assert.match(wikiPage, /selectedNodeId/, '고시해석 위키트리는 선택한 트리 노드에 따라 문서를 바꿔야 합니다.');
  assert.match(wikiPage, /NOTICE_SECTIONS/, '1차 고시 노드는 관련 법령·고시 안내 장별 내용을 렌더링해야 합니다.');
  ...
  assert.match(wikiCss, /\.wikiShell\s*\{[\s\S]*grid-template-columns/, '고시해석 위키트리는 좌측 트리와 우측 문서를 나눠야 합니다.');
  assert.match(wikiCss, /\.treeRail/, '고시해석 위키트리에는 좌측 트리 영역이 있어야 합니다.');
  assert.match(wikiCss, /\.documentPanel/, '고시해석 위키트리에는 우측 문서 영역이 있어야 합니다.');
  assert.match(wikiCss, /\.navTree/, '고시해석 위키트리에는 실제 트리 네비게이션 스타일이 있어야 합니다.');
