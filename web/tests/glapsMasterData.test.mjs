...
test('GLAPS 마스터는 운송경로 외 전 시트를 원본행과 항목 코드로 파싱한다', () => {
  const parsed = parseGlapsMasterSheets([
    ...
    },
    {
      name: '컨테이너규격',
      rows: [
        ['ISO코드', '세관코드', '설명 (Description)', 'GLAPS 등록 KEY'],
        ['4510', '40HC', "40' HIGH CUBIC", ''],
      ],
    },
    {
      name: '프로코드',
      rows: [
        ['등록구분', 'GLAPS 코드', 'GLAPS 포트', '명칭(GLOVE)', 'KD보낼값'],
        ['로케이션', 'PNITC', 'KRBNP', '부산신항국제터미널 (PNIT)', '[KRBNP]BUSAN(NEW)'],
      ],
    },
  ]);
  ...
  assert.equal(parsed.aliases.some(alias => alias.aliasType === 'port' && alias.sourceName === 'KRBNP' && alias.glapsCode === 'KRBNP'), true);
