...
test('GLAPS 마스터는 운송경로 외 전 시트를 원본행과 항목 코드로 파싱한다', () => {
  ...
  assert.equal(parsed.sheetRows.some(row => row.sheetName === '프로코드' && row.rowPayload.포트코드 === 'USLGB'), true);
  assert.equal(parsed.aliases.some(alias => alias.aliasType === 'container_type' && alias.glapsCode === '40HC'), true);
  assert.equal(parsed.aliases.some(alias => alias.aliasType === 'port' && alias.glapsCode === 'USLGB'), true);
  assert.deepEqual(parsed.sourceSheets, ['운송경로', '컨테이너규격', '프로코드']);
});
