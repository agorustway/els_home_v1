...
test('완료 경로 패널은 평균속도 대신 운행거리와 신뢰 최고속도를 표시한다', () => {
  const statsStart = mapSource.indexOf('function buildRouteStats');
  const statsEnd = mapSource.indexOf('// ─── 외부 공개 API', statsStart);
  const statsBody = mapSource.slice(statsStart, statsEnd);
  const routeStart = mapSource.indexOf('export async function showTripRouteOnMap');
  const routeEnd = mapSource.indexOf('/** 경로 표시 초기화 */', routeStart);
  const routeBody = mapSource.slice(routeStart, routeEnd);

  assert.ok(statsBody.includes('computeReliableMaxSpeed(points)'), 'route stats should compute max speed from timed coordinates');
  assert.ok(routeBody.includes('<b>운행거리</b>'), 'completed route panel should show driven distance');
  assert.equal(routeBody.includes('<b>평균속도</b>'), false, 'completed route panel should not show average speed');
});

test('저속 회전 센서 포인트는 GPS_TURN 마커로 서버에 남긴다', () => {
  const gyroStart = floatingServiceSource.indexOf('private void startGyroListener');
  const gyroEnd = floatingServiceSource.indexOf('private long getLocationTimeMs', gyroStart);
  const gyroBody = floatingServiceSource.slice(gyroStart, gyroEnd);

  assert.ok(floatingServiceSource.includes('sendSensorTurnMarker'), 'native service should have a dedicated sensor turn marker helper');
  assert.ok(floatingServiceSource.includes('"GPS_TURN"'), 'sensor turn events should be sent as GPS_TURN markers');
  assert.ok(floatingServiceSource.includes('SENSOR_TURN_MIN_SPEED_KPH = 1f'), 'idle shake should still be ignored');
  assert.equal(gyroBody.includes('mLastSpeedKph < 6f'), false, 'low-speed driving turns must not be skipped');
  assert.ok(floatingServiceSource.includes('jsonBody.put("gyro", mLastGyroMagnitude)'), 'turn marker should include gyro magnitude when possible');
});
...
test('앱 최소화 시 기존 플로팅 위젯을 보이고 복귀 시 숨긴다', () => {
  ...
  assert.ok(onResumeBody.includes('stopService(new Intent(this, FloatingWidgetService.class))'), 'resume without an active trip should stop leftover overlay service');
  ...
  assert.ok(onPauseBody.includes('stopService(new Intent(this, FloatingWidgetService.class))'), 'pause without an active trip should not leave a stale overlay service');
...
test('운행 종료는 오버레이/GPS만 정리하고 앱 화면은 유지한다', () => {
  ...
  assert.ok(endTripBody.includes('await stopOverlayService()'), 'endTrip should wait until native overlay service receives the stop request');
  ...
  assert.ok(overlayPluginSource.includes('STOP_OVERLAY_SERVICE'), 'stopService should leave a native stop latch for sticky service restarts');
