...
test('수식형 자동 템플릿은 원본 행수와 출력 행수를 분리한다', () => {
  ...
  assert.match(
    buildHelperIndexFormula('GLAPS컨테이너배차관리', 'X', '$B2'),
    /IF\(INDEX\('GLAPS컨테이너배차관리'!\$X:\$X,\$B2\)="","",INDEX/,
  );
  assert.equal(
    wrapFormulaWithInputGuard(
  ...
  assert.match(scriptSource, /buildSourcePlan/);
  assert.match(scriptSource, /sourceMatrix/);
...
test('수식형 자동 템플릿은 병합된 원본 시트를 복사하지 않고 평탄화 표로 넣는다', () => {
  const psSource = fs.readFileSync(path.join(webRoot, 'scripts/build-glaps-container-formula-workbook.ps1'), 'utf8');

  assert.match(psSource, /sourceMatrix/);
  assert.match(psSource, /\.Range\(\$plan\.sourceRange\)\.Value2 = \$sourceMatrix/);
  assert.match(psSource, /\$copiedSourceSheet\.Name = 'GLAPS컨테이너배차관리'/);
  assert.match(psSource, /\$containerColumnNumber = \[int\]\$plan\.containerColumnNumber/);
  assert.doesNotMatch(psSource, /\.Copy\(\$missing, \$elsSheet\)/);
});
