...
test('아산 종합실적 화면은 사장 관점 핵심 KPI를 빠른 뷰로 제공한다', () => {
  ...
  assert.match(page, /<AsanSummaryPerformance \/>/);
  assert.doesNotMatch(page, /performanceSearchHandoff/);
  assert.doesNotMatch(page, /openPerformanceSearchTab/);
  assert.doesNotMatch(page, /onOpenAnnual=/);
  assert.doesNotMatch(page, /onOpenMonthly=/);
  assert.doesNotMatch(page, /searchHandoff=\{performanceSearchHandoff/);
  ...
  assert.match(component, /컨테이너 운송 통합실적/);
  ...
  assert.match(component, /통합 이익/);
  assert.match(component, /선택 범위 합산 구조/);
  assert.match(component, /청록 막대 = 매출/);
  assert.match(component, /파란 선 = 이익/);
  assert.match(component, /연도별 매출·이익 흐름/);
  assert.match(component, /월별 매출·이익 흐름/);
  assert.match(component, /일별 매출·이익 흐름/);
  assert.match(component, /그래프 단위/);
  assert.match(component, /isSelected \? '현재'/);
  ...
  assert.match(component, /원가율/);
  assert.match(component, /최근월/);
  assert.match(component, /경영 판단/);
  assert.match(component, /청구처는 매출 기준, 지급처는 매입 기준/);
  assert.match(component, /normalizeExecutiveSignal/);
  assert.doesNotMatch(component, /function executiveSignalSearch/);
  assert.match(component, /EXECUTIVE_SIGNAL_TITLE_MAP/);
  assert.doesNotMatch(component, /openMonthly\?\.\(handoff\)/);
  assert.doesNotMatch(component, /summarySignalButton/);
  assert.doesNotMatch(component, /손익률/);
  assert.doesNotMatch(component, /손익/);
  assert.match(summaryUtils, /buildMarginSignal\('이익률 우수 청구처', billingSection, 'high', 'revenue'\)/);
  assert.match(summaryUtils, /buildMarginSignal\('이익률 우수 지급처', payeeSection, 'high', 'purchase'\)/);
  assert.match(summaryUtils, /buildMarginSignal\('이익률 점검 청구처', billingSection, 'low', 'revenue'\)/);
  assert.match(summaryUtils, /buildMarginSignal\('이익률 점검 지급처', payeeSection, 'low', 'purchase'\)/);
  assert.match(component, /당사 \/ 협력사 비교/);
  ...
  assert.doesNotMatch(component, /상세는 월간실적/);
  assert.doesNotMatch(component, /openAnnual/);
  assert.doesNotMatch(component, /openMonthly/);
  ...
  assert.match(component, /summaryYearCard/);
  assert.match(component, /summaryMiniRow/);
  assert.match(component, /ExecutiveSourceTable/);
  assert.match(component, /마감자료 구분/);
  assert.doesNotMatch(component, /원장 신뢰도/);
  ...
  assert.match(css, /\.summarySourceHead span:not\(:first-child\)\s*{[\s\S]*text-align: right/);
  assert.doesNotMatch(css, /\.summarySignalButton/);
  assert.match(css, /\.summaryYearMatrix/);
  assert.match(css, /\.summaryYearCard/);
  assert.match(css, /\.summaryMiniRow/);
...
test('종합실적 합산 유틸은 연간과 월간의 같은 기간을 더하고 경영 신호를 만든다', () => {
        ...
        breakdowns: [
          {
            column: '청구처',
            items: [
              { label: '우수화주', revenue: 200, purchase: 120, profit: 80, rowCount: 2, monthly: [{ period: '2026-04', revenue: 200, purchase: 120, profit: 80, rowCount: 2 }], daily: [{ date: '2026-04-10', period: '2026-04', revenue: 80, purchase: 40, profit: 40, rowCount: 1 }] },
              { label: '점검화주', revenue: 300, purchase: 294, profit: 6, rowCount: 3, monthly: [{ period: '2026-04', revenue: 300, purchase: 294, profit: 6, rowCount: 3 }], daily: [{ date: '2026-04-10', period: '2026-04', revenue: 20, purchase: 19, profit: 1, rowCount: 1 }] },
            ],
          },
          {
            column: '지급처',
            items: [
              { label: '우수지급처', revenue: 220, purchase: 110, profit: 110, rowCount: 2, monthly: [{ period: '2026-04', revenue: 220, purchase: 110, profit: 110, rowCount: 2 }], daily: [{ date: '2026-04-10', period: '2026-04', revenue: 90, purchase: 45, profit: 45, rowCount: 1 }] },
              { label: '점검지급처', revenue: 180, purchase: 176, profit: 4, rowCount: 2, monthly: [{ period: '2026-04', revenue: 180, purchase: 176, profit: 4, rowCount: 2 }], daily: [{ date: '2026-04-10', period: '2026-04', revenue: 10, purchase: 9, profit: 1, rowCount: 1 }] },
            ],
          },
        ],
        vehiclePerformance: [
        ...
        breakdowns: [
          {
            column: '청구처',
            items: [
              { label: '월간우수화주', revenue: 100, purchase: 60, profit: 40, rowCount: 1, monthly: [{ period: '2026-04', revenue: 100, purchase: 60, profit: 40, rowCount: 1 }], daily: [{ date: '2026-04-10', period: '2026-04', revenue: 40, purchase: 20, profit: 20, rowCount: 1 }] },
            ],
          },
          {
            column: '운송사',
  ...
  assert.ok(summary.executiveSignals.some(item => item.title === '자사 비율' && item.value.includes('자사')));
  assert.ok(summary.executiveSignals.some(item => item.title === '이익률'));
  assert.ok(summary.executiveSignals.some(item => item.title === '이익률 우수 청구처'));
  assert.ok(summary.executiveSignals.some(item => item.title === '이익률 우수 지급처'));
  assert.ok(summary.executiveSignals.some(item => item.title === '이익률 점검 청구처'));
  assert.ok(summary.executiveSignals.some(item => item.title === '이익률 점검 지급처'));
  const highBillingSignal = summary.executiveSignals.find(item => item.title === '이익률 우수 청구처');
  const highPayeeSignal = summary.executiveSignals.find(item => item.title === '이익률 우수 지급처');
  const lowPayeeSignal = summary.executiveSignals.find(item => item.title === '이익률 점검 지급처');
  ...
  assert.ok(!summary.executiveSignals.some(item => item.title === '이익률 점검 차량'));
  ...
  assert.ok(scopedMonth.executiveSignals.some(item => item.title === '이익률 점검 지급처'));
  ...
  assert.equal(dashboardView.breakdowns.find(item => item.column === '운송사').items[0].label, '월간운송사');
