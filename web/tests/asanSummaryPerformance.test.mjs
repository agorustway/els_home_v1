...
test('아산 종합실적 화면은 사장 관점 핵심 KPI와 상세 탭 이동을 제공한다', () => {
  ...
  assert.match(page, /performanceSearchHandoff/);
  assert.match(page, /const openPerformanceSearchTab = \(tab, handoff = null\) =>/);
  assert.match(page, /onOpenAnnual=\{\(handoff\) => openPerformanceSearchTab\('annual-performance', handoff\)\}/);
  assert.match(page, /onOpenMonthly=\{\(handoff\) => openPerformanceSearchTab\('monthly-performance', handoff\)\}/);
  ...
  assert.match(component, /function executiveSignalSearch/);
  assert.match(component, /EXECUTIVE_SIGNAL_TITLE_MAP/);
  assert.match(component, /if \(!title\.includes\('청구처'\) && !title\.includes\('지급처'\)\) return null/);
  assert.match(component, /openMonthly\?\.\(handoff\)/);
  assert.match(component, /summarySignalButton/);
  ...
  assert.match(css, /\.summarySignalButton/);
