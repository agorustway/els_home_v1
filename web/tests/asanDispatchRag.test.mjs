...
test('운송사 자체 상차지별 질문은 같은 행의 다른 업체 수량을 더하지 않는다', () => {
  const intent = parseAsanDispatchIntent('오늘 대신 운송사 자체의 상차지별 배차수량만 알려줘', { now });
  const casualIntent = parseAsanDispatchIntent('상차지별 올 대신수량만 말할수 없어?', { now });
  const result = buildAsanDispatchRagText([
    {
      target_date: '2026-05-19',
      type: 'glovis',
      headers: ['담당자', '작업지', '오더(계)', '배차정보', '부산', '기타/철송', '중부', '부곡'],
      data: [
        ['조윤진', '글로비스KD센터', '10', '08 09 10', '대신5', '신승5', '', ''],
        ['강수지', 'KCC글라스', '7', '13 14', '', '대신4', '셔틀3', ''],
        ['김태영', '모원리공장', '1', '15', '', '', '', '대신1'],
        ['운건하', '타업체작업', '2', '16', '이지2', '', '', ''],
      ],
      comments: {},
  ...
  assert.equal(intent.shouldQuery, true);
  assert.deepEqual(intent.carrierFilters, ['대신']);
  assert.deepEqual(intent.groupBy, ['region']);
  assert.deepEqual(intent.specificKeywords, []);
  assert.deepEqual(casualIntent.carrierFilters, ['대신']);
  assert.deepEqual(casualIntent.groupBy, ['region']);
  assert.deepEqual(casualIntent.specificKeywords, []);
  assert.match(result.text, /필터 기준: 운송사=대신/);
  assert.match(result.text, /정답 후보: 실제 배차 10대 \(오더 10대\)/);
  assert.match(result.text, /상차지별 답변 자료: 부산\(5대\), 기타\/철송\(4대\), 부곡\(1대\)/);
  assert.match(result.text, /매칭 운송사: 대신\(10대\)/);
  assert.doesNotMatch(result.text, /신승 5대/);
  assert.doesNotMatch(result.text, /셔틀 3대/);
  assert.doesNotMatch(result.text, /이지 2대/);
});

test('운송사와 상차지 동시 질문은 해당 지역 셀의 해당 업체 항목만 합산한다', () => {
  const intent = parseAsanDispatchIntent('오늘 부산 대신 배차 몇대야?', { now });
  const result = buildAsanDispatchRagText([
    {
      target_date: '2026-05-19',
      type: 'glovis',
      ...
      data: [
        ['조윤진', '글로비스KD센터', '10', '08 09', '대신5,이지1', '대신4'],
      ],
      comments: {},
    },
  ], intent);
  ...
  assert.deepEqual(intent.carrierFilters, ['대신']);
  assert.deepEqual(intent.regionFilters, ['부산']);
  assert.match(result.text, /정답 후보: 실제 배차 5대 \(오더 5대\)/);
  assert.match(result.text, /매칭 상차지: 부산\(5대\)/);
  assert.doesNotMatch(result.text, /아산 대신 4대/);
