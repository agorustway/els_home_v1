...
test('상세배차 행 변환은 요청 컬럼 순서를 유지한다', () => {
  ...
  const startCodeIndex = DISPATCH_DETAIL_HEADERS.indexOf('반출지(출발)코드');
  assert.equal(detailLineToRow({
    startLocation: '부산신항',
    billingStartLocation: '부산신항',
  })[startCodeIndex], '');
  assert.equal(detailLineToRow({
    startLocation: '부산신항',
    billingStartLocation: 'KRBNP',
  })[startCodeIndex], 'KRBNP');
...
test('배차변동 GLAPS 업로드 양식은 삭제 변동건을 신규 업로드 대상에서 제외한다', () => {
  ...
  const changeBaseRow = DISPATCH_CHANGE_DETAIL_HEADERS.map(header => baseRow[DISPATCH_DETAIL_HEADERS.indexOf(header)] || '');
  const addRow = [...changeBaseRow, '추가', '미확인', '2026-05-25 09:00', '', ''];
  const deleteRow = [...changeBaseRow, '삭제', '미확인', '2026-05-25 09:01', '', ''];
...
test('배차변동 표 헤더는 상세 수정일시 대신 발생일시만 표시한다', () => {
  assert.equal(DISPATCH_DETAIL_HEADERS.includes('수정일시'), true);
  assert.equal(DISPATCH_CHANGE_HEADERS.includes('수정일시'), false);
  assert.equal(DISPATCH_CHANGE_HEADERS.includes('발생일시'), true);
});
...
test('배차변동 비교는 선사 라인이 바뀌면 변경 이벤트로 감지한다', () => {
  const headers = ['작업일자', '구분', '화주', '작업지', '선적', '고객사', '포트', '라인', 'TYPE', '부산', 'BKG1'];
  const beforeLines = buildDispatchDetailLines({
    headers,
    rows: [['2026-05-26', '수출', '글로비스', 'KCC글라스', '부산신항', 'KAGA', 'USSAV', 'EMC', '40HC', '민경1', 'BKG-A']],
  ...
  assert.equal(events[0].changeType, 'change');
  assert.equal(events[0].quantityDelta, 0);
  assert.deepEqual(changedHeaders, ['라인']);
});
...
test('배차변동 비교는 하차지와 DG/RF 변경을 변경 이벤트로 감지한다', () => {
  const baseLine = {
    lineNo: 1,
    workDate: '2026-05-29',
    direction: '수출',
  ...
  const events = diffDispatchChangeLines([beforeLine], [afterLine], { occurredAt: '2026-05-29T01:00:00Z' });
  const changedHeaders = getDispatchChangeDiffHeaders(events[0].beforeSnapshot.rowValues, events[0].afterSnapshot.rowValues);

  assert.equal(events.length, 1);
  assert.equal(events[0].changeType, 'change');
  assert.deepEqual(changedHeaders, ['하차지(선적)', DISPATCH_DETAIL_DG_HEADER, DISPATCH_DETAIL_RF_HEADER]);
});
