...
test('상세배차는 지역칸 메모의 단일 업체 시간을 수량별 라인에 배정한다', () => {
  ...
  assert.deepEqual(lines.map(line => line.dispatchTime), ['13', '14', '15']);
  assert.deepEqual(
...
test('상세배차는 지역칸 메모의 업체별 시간을 업체 수량에 맞춰 배정한다', () => {
  ...
  assert.deepEqual(lines.map(line => line.dispatchTime), ['16', '18', '19', '13', '14']);
});
...
test('상세배차 행은 GLAPS 업로드 첫 시트 양식으로 변환하고 같은 부킹은 수량으로 묶는다', () => {
  ...
  assert.equal(byHeader['배차요청시간'], '13');
