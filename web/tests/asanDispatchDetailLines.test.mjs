...
test('기타/철송과 중부는 상세배차에서 선택 필요 상태로 둔다', () => {
  const headers = ['작업일자', '구분', '화주', '작업지', '기타/철송', '중부'];
  const rows = [['2026-05-23', '수출', '글로비스', 'KCP', '광진1', '선진2']];

  const lines = buildDispatchDetailLines({ headers, rows });

  assert.equal(lines.length, 3);
  assert.deepEqual(lines.map((line) => line.startLocation), ['', '', '']);
  assert.equal(summarizeDispatchDetailLines(lines).manualStartLocationCount, 3);
});
...
test('상세배차 행 변환은 요청 컬럼 순서를 유지한다', () => {
  ...
  assert.deepEqual(detailLineToRow({
    ...
    targetVessel: 'TV',
    note: 'N',
  }), ['26/5/23', '수출', '글로비스', '부산신항', 'KCP', '부산신항', 'HD', 'USLGB', 'MSC', '40HC', '민경', 'B1', 'B2', 'B3', 'TV', 'N']);
});
