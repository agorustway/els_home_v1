...
test('상세배차 DG.RF는 TYPE에 R이 있으면 기본 Y, 아니면 N으로 표시한다', () => {
  assert.equal(inferDispatchDgRfFlag('40HC'), 'N');
  assert.equal(inferDispatchDgRfFlag('20RF'), 'Y');
  assert.equal(inferDispatchDgRfFlag('22RE'), 'Y');
  assert.equal(inferDispatchDgRfFlag('45RH'), 'Y');
  assert.equal(normalizeDispatchDgRfValue('', '40HC'), 'N');
  assert.equal(normalizeDispatchDgRfValue('', '20RF'), 'Y');
  assert.equal(normalizeDispatchDgRfValue('n', '20RF'), 'N');
});
...
test('GLAPS 업로드 냉동 여부는 상세배차 DG.RF 값을 사용한다', () => {
  const detailRow = detailLineToRow({
    direction: '수출',
    containerType: '20RF',
    glapsTypeCode: '2232',
  ...
  assert.equal(detailRow[DISPATCH_DETAIL_HEADERS.indexOf(DISPATCH_DETAIL_DG_RF_HEADER)], 'Y');
...
test('배차변동 비교는 같은 운송조건 안의 업체 교체를 삭제/추가로 감지한다', () => {
  ...
  assert.ok(events.some(event => event.changeType === 'delete' && event.beforeSnapshot.rowValues[companyIdx] === '칸'));
  assert.ok(events.some(event => event.changeType === 'add' && event.afterSnapshot.rowValues[companyIdx] === '이지'));
});
