...
test('아산 월간실적 이월 순환은 전월이월 반영분과 익월이월 발생분을 분리한다', () => {
  const headers = ['이월여부', '작업일자', '청구처', '청구', '지급처', '하불', '이월구분', '청구_1', '하불_1'];
  const rows = [
    ['수출', '2026-04-01', '일반화주', '1,000,000', '일반운송사', '800,000', '', '', ''],
    ['이월', '2026-03-28', '전월화주', '500,000', '전월운송사', '420,000', '', '', ''],
    ['수출', '2026-04-26', '익월화주', '', '익월운송사', '', '5월이월(청구만)', '300,000', '0'],
    ['이월', '2026-03-31', '재이월화주', '200,000', '재이월운송사', '150,000', '5월이월', '200,000', '150,000'],
  ];

  const cycle = buildMonthlyCarryoverCycle(headers, rows, { period: '2026-04' });
  assert.equal(cycle.basis, '마감월');
  assert.equal(cycle.incoming.revenue, 700000);
  assert.equal(cycle.incoming.purchase, 570000);
  assert.equal(cycle.incoming.profit, 130000);
  assert.equal(cycle.incoming.rowCount, 2);
  assert.equal(cycle.outgoing.revenue, 500000);
  assert.equal(cycle.outgoing.purchase, 150000);
  assert.equal(cycle.outgoing.profit, 350000);
  assert.equal(cycle.outgoing.rowCount, 2);
  assert.equal(cycle.targetPeriods[0].period, '2026-05');
  assert.equal(cycle.outgoing.clientItems[0].name, '익월화주');
  assert.equal(cycle.incoming.clientItems[0].name, '전월화주');
});
...
test('아산 월간실적 Next 라우트는 Supabase monthly 조회와 NAS 백엔드 동기화를 제공한다', () => {
  ...
  assert.match(dbReader, /rowsQuery = rowsQuery\.is\('is_current', true\)\.in\('file_path'/);
  assert.match(dbReader, /\.order\('file_path', \{ ascending: true \}\)[\s\S]*\.order\('sheet_name', \{ ascending: true \}\)[\s\S]*\.order\('row_index', \{ ascending: true \}\)/);
  assert.match(dbReader, /row_data/);
  assert.match(dbReader, /monthlyFileSlots/);
  assert.match(dbReader, /monthlyReports/);
  assert.match(dbReader, /carryoverCycle/);
  assert.match(dbReader, /전월이월 반영분/);
  assert.match(dbReader, /익월이월 발생분/);
  ...
  assert.match(dbReader, /monthlyBasis: '마감월'/);
...
test('아산 월간실적 화면은 파일 설정 저장 후 자동 동기화와 이월 월 슬롯을 제공한다', () => {
  ...
  assert.match(component, /전월이월/);
  assert.match(component, /평균청구\(VAN\)/);
  assert.doesNotMatch(component, /<span>청구이월<\/span>/);
  assert.doesNotMatch(component, /<span>건당 청구<\/span>/);
