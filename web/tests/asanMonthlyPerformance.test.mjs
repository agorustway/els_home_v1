...
test('아산 월간실적 화면은 파일 설정 저장 후 자동 동기화와 이월 월 슬롯을 제공한다', () => {
  ...
  assert.doesNotMatch(component, /청구픽업별/);
  assert.doesNotMatch(component, /포트별/);
  assert.doesNotMatch(component, /노선별/);
  assert.doesNotMatch(component, /이월구분별/);
  assert.doesNotMatch(component, /계약별/);
  assert.match(component, /청구처 이월/);
  assert.match(component, /이월청구/);
  assert.match(component, /이월하불/);
  ...
  assert.match(css, /\.carryoverClientPanel/);
  assert.match(css, /\.carryoverClientRows/);
