...
test('아산 연간실적 Next 라우트는 NAS 백엔드로 프록시한다', () => {
  ...
  assert.match(dbReader, /asan_annual_route_unit_price_rows/);
  assert.match(dbReader, /routeUnitPrice/);
  assert.match(dbReader, /route-unit-price:v2/);
...
test('아산 연간실적 Supabase SQL은 누적 원장과 현재 조회 인덱스를 만든다', () => {
  ...
  assert.match(routeUnitSql, /CREATE OR REPLACE FUNCTION public\.asan_annual_route_unit_price_rows/);
  ...
  assert.match(routeUnitSql, /GRANT EXECUTE ON FUNCTION public\.asan_annual_route_unit_price_rows/);
});
...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(component, /매출열 · 청구처 · 지급처 · TYPE · 청구\/하불/);
