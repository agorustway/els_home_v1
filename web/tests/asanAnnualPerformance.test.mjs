...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(component, /청구\/하불 금액과 매출·지역·작업지·운송사·구분·픽업·청구픽업·선적·TYPE·청구처·하불처 조건/);
  ...
  assert.match(component, /routeUnitFilterPanel/);
  assert.match(component, /routeUnitFilterList/);
  assert.match(component, /routeUnitFilterChips/);
  assert.match(component, /필터 목록/);
  ...
  assert.match(css, /\.routeUnitTableTools\s*{[\s\S]*grid-template-columns: minmax\(220px, 1fr\) 190px auto auto/);
  ...
  assert.match(css, /\.routeUnitFilterPanel/);
  assert.match(css, /\.routeUnitFilterList\s*{[\s\S]*grid-template-columns: repeat\(6, minmax\(120px, 1fr\)\)/);
  assert.match(css, /\.routeUnitFilterChips/);
