...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(css, /\.routeUnitTableTools\s*{[\s\S]*grid-template-columns: minmax\(260px, 520px\) minmax\(0, 1fr\) auto/);
  assert.match(css, /\.routeUnitHiddenZone\s*{[\s\S]*border: 1px dashed #cbd5e1/);
  assert.match(css, /\.routeUnitHiddenZoneActive/);
  assert.match(css, /\.routeUnitSearchBox\s*{[\s\S]*grid-template-columns: 34px minmax\(0, 1fr\)/);
