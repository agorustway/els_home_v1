...
test('아산 연간실적 기본 조회는 Supabase 원장만 읽고 빈 DB는 동기화 대기로 응답한다', () => {
  ...
  assert.match(dbReader, /else \{\s*query = query\.is\('is_current', true\);/);
...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(component, /이익률 높은순/);
  assert.match(component, /label: '이익률'/);
  assert.match(component, /routeUnitProfitRateOf/);
  assert.match(component, /formatPercent\(item\.unitProfitRate, 2\)/);
