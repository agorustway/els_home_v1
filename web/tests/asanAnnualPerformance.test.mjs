...
test('아산 연간실적 Next 라우트는 NAS 백엔드로 프록시한다', () => {
  ...
  assert.match(dbReader, /datasetBasis: '월간실적 current 원장'/);
  ...
  assert.match(dbReader, /maxGroups = 5000/);
  ...
  assert.match(dbReader, /매출.*지역.*작업지.*운송사\(명의\).*구분.*픽업.*청구픽업.*선적.*청구처.*하불처/);
...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(component, /월간실적 current 원장만 사용/);
  assert.match(component, /월간 금액표/);
  assert.match(component, /청구\/하불 금액과 매출·지역·작업지·운송사·구분·픽업·청구픽업·선적·청구처·하불처 조건/);
  ...
  assert.match(component, /const \[routeUnitScope, setRouteUnitScope\] = useState\('month'\)/);
  ...
  assert.match(component, /금액, 지역, 작업지, 운송사, 청구처/);
  ...
  assert.match(css, /\.routeUnitControls\s*{[\s\S]*grid-template-columns: minmax\(220px, 1fr\) auto 160px auto/);
  ...
  assert.match(css, /\.routeUnitHead,\s*\.routeUnitRow\s*{[\s\S]*min-width: 1420px/);
