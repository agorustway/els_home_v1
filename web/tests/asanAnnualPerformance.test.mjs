...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(component, /손익 최저/);
  assert.match(component, /trendSummaryGrid/);
  assert.match(component, /평균 손익률/);
  ...
  assert.match(component, /요일별 매출·손익 비중/);
  assert.match(component, /weekdayShareBoard/);
  assert.match(component, /매출 비중/);
  assert.match(component, /손익 기여/);
  ...
  assert.match(css, /\.weekdayShareBoard\s*{/);
  assert.match(css, /\.weekdayShareRail\s*{[\s\S]*border-radius: 999px/);
  assert.match(css, /\.weekdayShareSegment\s*{[\s\S]*text-overflow: ellipsis/);
  assert.match(css, /\.weekdayDiagram\s*{[\s\S]*grid-template-columns: repeat\(7, minmax\(0, 1fr\)\)/);
  assert.match(css, /\.weekdayInsightStrip\s*{[\s\S]*grid-template-columns: repeat\(4, minmax\(0, 1fr\)\)/);
