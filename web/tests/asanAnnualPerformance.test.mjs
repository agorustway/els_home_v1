...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(css, /\.marketFlowChartWrap\s*{[\s\S]*height: 220px/);
  assert.match(css, /\.marketFlowSvg\s*{[\s\S]*width: max\(100%, 1500px\)/);
  ...
  assert.match(css, /\.trendMarkerLabel\s*{[\s\S]*paint-order: stroke/);
  ...
  assert.match(css, /@media \(max-width: 480px\)[\s\S]*\.marketFlowSvg\s*{[\s\S]*width: 900px/);
