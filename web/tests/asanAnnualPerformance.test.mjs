...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(component, /선택한 묶음 항목만 기준/);
  ...
  assert.match(component, /routeUnitGroupBar/);
  assert.match(component, /금액만/);
  ...
  assert.match(css, /\.routeUnitGroupBar\s*{[\s\S]*grid-column: 1 \/ -1/);
  assert.match(css, /\.routeUnitGroupChips button/);
  assert.match(css, /\.routeUnitGroupChipActive/);
