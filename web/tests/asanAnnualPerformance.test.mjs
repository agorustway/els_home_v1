...
test('아산 연간실적 Next 라우트는 NAS 백엔드로 프록시한다', () => {
  ...
  assert.match(dbReader, /routeUnitAmountCacheSignatureState/);
  assert.match(dbReader, /branch_performance_monthly_route_unit_amount_cache/);
  assert.match(dbReader, /routeUnitAmountCacheState/);
  ...
  assert.match(dbReader, /dashboardType: 'annual-route-unit-price'/);
  assert.match(dbReader, /refresh: dashboardRefreshRequested\(searchParams\)/);
...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(component, /ROUTE_UNIT_FILTER_DEBOUNCE_MS/);
  assert.match(component, /const \[unitFilterInput, setUnitFilterInput\] = useState\(''\)/);
  assert.match(component, /routeUnitFilterSnapshotRef/);
  assert.match(component, /routeUnitColumnFiltersKey/);
  assert.match(component, /setTimeout\(\(\) => \{\s*setUnitFilter\(unitFilterInput\.trim\(\)\);/);
