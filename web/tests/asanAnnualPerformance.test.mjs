...
test('아산 연간실적 Next 라우트는 NAS 백엔드로 프록시한다', () => {
  ...
  assert.match(dbReader, /asan_performance_route_unit_price_rows/);
  assert.match(dbReader, /routeUnitPrice/);
  assert.match(dbReader, /route-unit-price:v3/);
  assert.match(dbReader, /routeUnitPriceSourceState/);
  assert.match(dbReader, /\.in\('dataset_type', \['annual', 'monthly'\]\)/);
  assert.match(dbReader, /monthlyOverridePeriods/);
  assert.match(dbReader, /statement timeout/);
  assert.match(dbReader, /ASAN_ROUTE_UNIT_RPC_ENABLED === '1'/);
  assert.match(dbReader, /applyRouteUnitLastMetric/);
  assert.match(dbReader, /unitBasis: 'last'/);
  assert.match(dbReader, /compareRouteUnitLastPrice/);
  assert.match(dbReader, /scope\.mode === 'month'[\s\S]*\.eq\('month_value'/);
  assert.match(dbReader, /픽업.*지역.*작업지.*하차/);
...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(component, /RouteUnitUnitTrendChart/);
  assert.match(component, /analysis: 'route-unit-price'/);
  assert.match(component, /unit_scope/);
  assert.match(component, /픽업-지역-작업지-하차/);
  assert.match(component, /건당 청구단가·하불단가·차액단가 변동만 추적/);
  assert.match(component, /청구단가/);
  assert.match(component, /하불단가/);
  assert.match(component, /차액단가/);
  assert.match(component, /formatRouteUnitWon/);
  assert.match(component, /normalizeRouteUnitLastDisplay/);
  assert.match(component, /routeUnitLatestSeriesItem/);
  ...
  assert.match(component, /최근 청구단가 높은순/);
  assert.match(component, /routeUnitMatchesFilter/);
  assert.match(component, /구간, 청구처, 지급처, TYPE/);
  assert.match(component, /LAST 기준/);
  assert.doesNotMatch(component, /평균 청구단가/);
  assert.doesNotMatch(component, /청구 합계/);
  ...
  assert.match(css, /\.routeUnitControls\s*{[\s\S]*grid-template-columns: minmax\(220px, 1fr\) auto 140px 150px auto/);
  assert.match(css, /\.routeUnitTableTools\s*{[\s\S]*grid-template-columns: minmax\(220px, 1fr\) 190px auto/);
  assert.doesNotMatch(css, /\.routeUnitRow > i/);
  assert.match(css, /\.routeUnitLayout\s*{[\s\S]*grid-template-columns: minmax\(0, 1\.25fr\) minmax\(360px, 0\.75fr\)/);
  assert.match(css, /\.routeUnitHead,\s*\.routeUnitRow\s*{[\s\S]*min-width: 1060px/);
  assert.match(css, /\.routeUnitTrendSvg\s*{[\s\S]*height: 250px/);
