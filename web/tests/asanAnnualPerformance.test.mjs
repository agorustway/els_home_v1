...
test('아산 연간실적 화면은 분석/테이블 탭, 파일 선택, 제목행 설정을 제공한다', () => {
  ...
  assert.match(component, /overviewEvidenceSections/);
  assert.match(component, /label: '작업지'/);
  assert.match(component, /label: '청구처'/);
  assert.match(component, /label: '노선'/);
  assert.match(component, /label: '구분'/);
