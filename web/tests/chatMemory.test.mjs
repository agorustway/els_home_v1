...
test('대화 메모리 API는 삭제 마커를 유지하고 늦은 저장을 막는다', () => {
  ...
  assert.match(route, /clearedAt:\s*data\.updated_at/);
  assert.doesNotMatch(route, /Date\.now\(\)\s*-\s*updatedAtTime\s*>\s*30000/);
  assert.doesNotMatch(route, /purged:\s*true/);
  assert.match(route, /messages:\s*\[\]/);
});

test('AI 대화 화면은 서버 삭제 마커를 받으면 로컬 옛 목록도 무효화한다', () => {
  const page = readFileSync(resolve(__dirname, '../app/(main)/employees/(intranet)/ask/page.js'), 'utf8');

  assert.match(page, /applyHistoryClearedMarker\(data\.clearedAt\)/);
  assert.match(page, /localStorage\.removeItem\(LS_KEY\)/);
  assert.match(page, /shouldUsePersistedSessions\(\{ sessions: finalSessions \|\| \[\], clearedAt: data\.clearedAt \}\)/);
  assert.doesNotMatch(page, /delay:\s*10000,\s*purge:\s*true/);
});
