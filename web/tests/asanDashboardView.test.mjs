...
test('아산 배차 날짜 탭은 유효 오더 없는 날짜를 비활성화한다', () => {
  ...
  assert.match(source, /import \{ buildAsanDashboardScope, getDispatchAssignedQty \} from '@\/utils\/asanDashboardView\.mjs';/);
...
test('아산 배차판은 GLAPS 검수용 상세배차내역 탭을 제공한다', () => {
  ...
  assert.match(source, /GLAPS_START_LOCATION_OPTIONS\.map/);
  ...
  assert.match(source, /GLAPS 업로드 전 검수용 상세 라인/);
...
test('아산 배차 통합현황은 비고 오른쪽 특이사항과 날짜별 가변 헤더를 표시한다', () => {
  ...
  assert.match(route, /"특이사항": getColAfter\(\["특이사항"\], \["비고"\]\)/);
