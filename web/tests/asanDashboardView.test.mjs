...
test('아산지점은 GLAPS 마스터 원장 화면과 DB 적용 SQL을 제공한다', () => {
  ...
  assert.match(masterSource, /selectedDuplicateIds/);
  ...
  assert.match(apiSource, /function omitBlankId/);
  assert.match(apiSource, /upsert\(upsertRows\.map\(omitBlankId\), \{ onConflict: 'id' \}\)/);
  ...
  assert.match(dispatchCss, /\.tableScroll\s*{[\s\S]*overflow: auto;[\s\S]*flex: 0 1 auto;[\s\S]*height: auto;[\s\S]*max-height: clamp\(260px, calc\(100vh - 440px\), 760px\);[\s\S]*scrollbar-gutter: stable;/);
