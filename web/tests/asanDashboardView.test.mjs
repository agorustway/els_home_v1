...
test('아산 배차 날짜 탭은 유효 오더 없는 날짜를 비활성화한다', () => {
  ...
  assert.match(source, /setActiveTab\(pickActiveIndex\(fullItems, currentTargetDate\)\)/);
...
test('아산 배차판은 GLAPS 검수용 상세배차내역 탭을 제공한다', () => {
  ...
  assert.match(source, /const lineShipperCode = getGlapsAliasCode\(glapsShipperCodeMap, line\.shipper\)/);
...
test('아산지점은 GLAPS 마스터 원장 화면과 DB 적용 SQL을 제공한다', () => {
  ...
  assert.match(masterSource, /매칭 키가 아니라 출처·용도·기본값/);
  ...
  assert.match(duplicateUtil, /최종코드\(BP\) 중복/);
  ...
  assert.match(apiSource, /function withRequiredId/);
  assert.match(apiSource, /crypto\.randomUUID\(\)/);
  assert.match(apiSource, /upsertRows\.map\(withRequiredId\)/);
  assert.match(apiSource, /insert\(withRequiredId\(dbRow\)\)/);
  assert.match(apiSource, /insert\(routeRows\.map\(withRequiredId\)\)/);
  assert.match(apiSource, /insert\(aliasRows\.map\(withRequiredId\)\)/);
  assert.match(apiSource, /insert\(sheetRows\.map\(withRequiredId\)\)/);
  ...
  assert.match(apiSource, /upsert\(upsertRows\.map\(withRequiredId\), \{ onConflict: 'id' \}\)/);
  ...
  assert.match(templateSource, /최종코드\(BP\)가 같은 행/);
  ...
  assert.match(dispatchCss, /\.table th\s*{[\s\S]*background: #1f5673;[\s\S]*padding: 3px 6px;[\s\S]*line-height: 1\.15;/);
  assert.match(dispatchCss, /\.table td\s*{[\s\S]*padding: 3px 6px;[\s\S]*line-height: 1\.18;/);
...
test('아산 배차 자동 갱신은 화면 위치를 유지하고 메모 변경도 DB 갱신 대상으로 본다', () => {
  ...
  const settingsButtonIdx = source.indexOf('onClick={() => setShowSettings(true)}>설정');
