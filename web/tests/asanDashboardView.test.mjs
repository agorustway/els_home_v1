...
test('아산 배차판은 GLAPS 검수용 상세배차내역 탭을 제공한다', () => {
  ...
  assert.match(source, /detailCarrierOverrides/);
  ...
  assert.match(source, /glapsPortCode: glapsPortCode \|\| line\.port \|\| ''/);
  assert.match(source, /DETAIL_ISSUE_FILTERS\.map/);
  assert.match(source, /DETAIL_CARRIER_CODE_DATALIST_ID/);
