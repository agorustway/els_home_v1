...
test('아산 현황판 예측 손익은 모원리공장과 서영모원리처럼 핵심어가 같은 작업지를 매칭한다', () => {
  ...
  const forecast = buildAsanDashboardFinancialForecast({
    sourceItems: [{
      ...
      data: [
        ['수출', '기아자동차MIP', '모원리공장', 'KAGA', 'HMM', '40HC', '1', '1', '이지1'],
      ],
    }],
    viewType: 'integrated',
    routeUnitPrice: {
  ...
  assert.equal(daily.revenue, 310_000);
  assert.equal(daily.purchase, 240_000);
...
test('아산 현황판 예측 손익 점검은 보정 사유를 카드에서 펼쳐 보여준다', () => {
  ...
  assert.match(css, /\.forecastIssueList\s*{[\s\S]*max-height: 112px;/);
  assert.match(css, /\.forecastIssueItem\s*{[\s\S]*grid-template-columns: 58px minmax\(0, 1fr\);/);
});
...
test('아산 현황판은 첫 화면 viewer cache를 우선 쓰고 없을 때만 전체 원장을 보강 조회한다', () => {
  ...
  assert.match(forecastApiSource, /function withTiming/);
  assert.match(forecastApiSource, /dispatchMetaMs/);
  assert.match(forecastApiSource, /dispatchRangeMs/);
  assert.match(forecastApiSource, /routeUnitRowsMs/);
  assert.match(forecastApiSource, /timings\.totalMs = Date\.now\(\) - startedAt/);
  assert.match(forecastApiSource, /routeUnitEngine: routeUnitPrice\?\.engine/);
  assert.match(forecastApiSource, /routeUnitGroupCount: routeUnitPrice\?\.summary\?\.returnedGroupCount/);
...
test('아산 현황판 도넛 범례는 항목별 점유율을 함께 표시한다', () => {
  ...
  assert.match(source, /function normalizeShareDonutName\(name, title\)/);
  assert.match(source, /title !== '상차지별 비율'/);
  assert.match(source, /function buildShareDonutEntries\(data, title\)/);
  assert.match(source, /pushShareDonutEntry\(top, '기타', otherTotal\)/);
  assert.doesNotMatch(source, /top\.push\(\['기타', otherTotal\]\)/);
...
test('아산지점은 GLAPS 마스터 원장 화면과 DB 적용 SQL을 제공한다', () => {
  ...
  assert.match(masterSource, /\[fetchData, refreshToken, searchInput\]/);
  ...
  assert.doesNotMatch(masterSource, /styles\.keyCell/);
  assert.match(masterSource, /\{ key: 'route_key'[\s\S]*styles\.protectedCell/);
  assert.doesNotMatch(masterSource, /\{ key: 'route_code'[\s\S]*styles\.keyCell/);
  assert.doesNotMatch(masterSource, /\{ key: 'glaps_code'[\s\S]*styles\.keyCell/);
  ...
  assert.match(masterSource, /조건: 화주사/);
  assert.match(masterSource, /조건: 경유지\(GLAPS\)/);
  assert.match(masterSource, /조건: 경유지\(ELS\)/);
  assert.match(masterSource, /기존값 무관/);
  assert.match(masterSource, /적용항목/);
  assert.match(masterSource, /적용값\(To\)/);
  ...
  assert.match(masterCss, /\.table td\.keyCell\s*{[\s\S]*background: #fff !important;[\s\S]*font-weight: 400;/);
  ...
  assert.match(masterCss, /\.duplicateRow td\s*{[\s\S]*background: #fff;/);
