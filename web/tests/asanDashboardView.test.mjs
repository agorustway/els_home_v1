...
test('아산 배차판은 GLAPS 검수용 상세배차내역 탭을 제공한다', () => {
  ...
  assert.match(changeApi, /delete-after-add:\$\{existing\.event_key/);
  assert.match(changeApi, /change_type: 'delete'/);
  assert.match(changeApi, /quantity_delta: -1/);
  assert.match(changeApi, /deleted_after_add/);
  assert.match(changeApi, /deletedAfterAddEventKey/);
  ...
  assert.match(source, /buildDeletedAfterAddPairKeySet/);
  assert.match(source, /추가취소쌍/);
...
test('아산지점은 GLAPS 마스터 원장 화면과 DB 적용 SQL을 제공한다', () => {
  ...
  assert.match(masterSource, /코드기준 미병합/);
  assert.match(masterSource, /selectedAliasIds/);
  assert.match(masterSource, /mergeAliasDuplicates/);
  assert.match(masterSource, /merge_by_glaps_code/);
  ...
  assert.match(apiSource, /action === 'merge_by_glaps_code'/);
