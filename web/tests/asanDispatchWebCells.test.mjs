...
test('글로비스 T 헤더는 WEB 표시에서 TYPE으로 정규화한다', () => {
  assert.deepEqual(
    normalizeDispatchHeadersForType(['구분', '라인', 'T', '오더'], 'glovis'),
    ['구분', '라인', 'TYPE', '오더'],
  );
  assert.deepEqual(
...
test('모비스 고객사 표시값은 국가와 도착항을 함께 병기한다', () => {
  assert.equal(
    formatMobisCountryPortLabel(['수출', '호주', '시드니', 'A00VAX'], ['구분', '국가명', '도착항', 'CODE']),
    '호주 시드니',
  );
  assert.equal(
    formatMobisCountryPortLabel(['수출', '독일', '', 'C00VDX'], ['구분', '국가', '도착항', 'CODE']),
    '독일',
  );
});
...
test('익명 TYPE 헤더 복구 후 통합/개별 WEB 셀 행 서명이 동일하다', () => {
  ...
  const glovisNamed = {
    headers: ['구분', '화주', '담당자', '작업지', '고객사', '포트', '특이사항(Nomi,구간)', '라인', 'TYPE', '오더'],
  };
  ...
  assert.equal(glovisAnon.headers[8], 'TYPE');
...
test('WEB 셀 오버레이는 행 서명이 흔들려도 같은 행번호의 저장값을 복구한다', () => {
  const buildMeta = createDispatchRowMetaBuilder({
    dispatchType: 'glovis',
    targetDate: '2026-05-22',
    headers,
  ...
  const cellMap = buildWebCellMap([
    {
      ...
      target_date: '2026-05-22',
      row_signature: 'stale-signature:001',
      field_key: 'BKG1',
      row_index: 0,
  ...
  assert.notEqual(meta.rowSignature, 'stale-signature:001');
  assert.equal(applied[9], 'RECOVER-BKG');
});

test('WEB 셀 행번호 복구는 같은 행의 최신 저장값을 우선한다', () => {
  const buildMeta = createDispatchRowMetaBuilder({
    dispatchType: 'mobis',
    targetDate: '2026-05-22',
    headers,
  });
  const row = ['수출', '모비스AS', '강주희', '모비스천안', '호주', '시드니', 'CMA', '40HC', '1', '', '', '', '', ''];
  const meta = buildMeta(row, 2);
  const cellMap = buildWebCellMap([
    {
      branch_id: 'asan',
      dispatch_type: 'mobis',
      target_date: '2026-05-22',
      row_signature: 'old-signature:001',
      field_key: 'BKG1',
      row_index: 2,
      row_context: meta.rowContext,
      value: 'OLD-BKG',
    ...
    },
    {
      ...
      dispatch_type: 'mobis',
      target_date: '2026-05-22',
      row_signature: 'new-signature:001',
      field_key: 'BKG1',
      row_index: 2,
      row_context: meta.rowContext,
      value: 'NEW-BKG',
