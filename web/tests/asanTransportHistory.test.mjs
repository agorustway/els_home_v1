...
test('아산 운송내역 전체 보기 페이지는 날짜와 입력순으로 정렬하고 SEQ를 다시 부여한다', () => {
  ...
  assert.deepEqual(page.data.map(row => row[1]), ['2026-01-05', '2026-02-01']);
});
...
test('아산 운송내역 전체 보기 페이지는 검색과 offset을 적용한다', () => {
  ...
  assert.deepEqual(page.data[0], ['2', '2026-01-03', 'KCC']);
});
...
test('아산 운송내역 화면은 전체/연도/더보기와 선적관리형 테이블 도구를 제공한다', () => {
  const source = fs.readFileSync(
    path.join(repoRoot, 'web/app/(main)/employees/branches/asan/AsanTransportHistory.js'),
    'utf8',
  );
  ...
  assert.match(source, /String\(currentYear \+ 1\)/);
