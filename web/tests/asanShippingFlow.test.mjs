...
test('선적관리 기본 진입은 월 필터를 비워 100건 페이지 로드를 유지한다', () => {
  const source = fs.readFileSync(
    path.join(repoRoot, 'web/app/(main)/employees/branches/asan/AsanShipping.js'),
    'utf8',
  );
  ...
  assert.match(source, /const SHIPPING_PAGE_SIZE = 100;/);
  assert.match(source, /months: \[\]/);
  assert.doesNotMatch(source, /getDefaultShippingMonthKeys\(\)/);
  assert.match(source, /pageSize = options\.pageSize \|\| SHIPPING_PAGE_SIZE/);
  assert.match(source, /Boolean\(dateFilter\.col && dateFilter\.months\?\.length > 0\)/);
});
...
test('선적관리 모바일 상단과 월 선택 영역은 한 화면 폭 안에서 정렬된다', () => {
  ...
  assert.match(css, /@media \(max-width: 768px\)[\s\S]*\.quickFilterBtn,[\s\S]*\.resultCountText \{[\s\S]*width: 100%;[\s\S]*min-width: 0;/);
});
