...
test('인트라넷 엑셀 공통 유틸은 시간 컬럼을 HH:MM 시간값으로 저장한다', () => {
  const workbook = new ExcelJS.Workbook();
  const sheet = addIntranetExportWorksheet(workbook, {
    sheetName: '상세배차내역',
  ...
  assert.equal(sheet.getCell('B2').value, 8 / 24);
  assert.equal(sheet.getCell('B2').numFmt, 'hh:mm');
  assert.equal(sheet.getCell('C2').value, (16 * 60 + 40) / 1440);
  assert.equal(sheet.getCell('C2').numFmt, 'hh:mm');
});
