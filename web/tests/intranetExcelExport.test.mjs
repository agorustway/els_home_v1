...
test('인트라넷 엑셀 공통 유틸은 요청 시 시간 컬럼을 HHMM 텍스트로 저장한다', () => {
  const workbook = new ExcelJS.Workbook();
  const sheet = addIntranetExportWorksheet(workbook, {
    sheetName: 'GLAPS_업로드',
    headers: ['업체명', '시간', '배차요청시간'],
  ...
  assert.equal(toIntranetExcelTimeText('08', { timeFormat: 'compact' }), '0800');
  assert.equal(toIntranetExcelTimeText('13:00', { timeFormat: 'compact' }), '1300');
  assert.equal(sheet.getCell('B2').value, '0800');
  assert.equal(sheet.getCell('B2').numFmt, '@');
  assert.equal(sheet.getCell('C2').value, '1300');
  assert.equal(sheet.getCell('C2').numFmt, '@');
});

test('인트라넷 엑셀 공통 유틸은 제목/메타 행을 제외하고 컬럼 폭을 맞춘다', () => {
  const workbook = new ExcelJS.Workbook();
  const sheet = addIntranetExportWorksheet(workbook, {
    title: '아산 선적관리 다운로드 제목이 매우 길어도 첫 번째 컬럼 폭을 밀어내면 안 됩니다',
    generatedAt: '다운로드 2026. 6. 2. 오전 11:01:28 / 32건',
    sheetName: '선적관리',
    headers: ['작업일', '작업지', '차량번호'],
    rows: [['2026-05-21', '글로비스KD센터3포장장', '충남99사6944']],
  });

  assert.ok(sheet.getColumn(1).width < 18);
  assert.ok(sheet.getColumn(2).width > sheet.getColumn(1).width);
  assert.equal(sheet.getCell('A1').value, '아산 선적관리 다운로드 제목이 매우 길어도 첫 번째 컬럼 폭을 밀어내면 안 됩니다');
  assert.equal(sheet.getCell('A3').value, '작업일');
});
