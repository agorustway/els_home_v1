...
test('AI 어시스턴트 버전과 소개는 함수형 메타에서 관리한다', () => {
  assert.equal(getAiAssistantVersion(), 'v5.14.210');

  const intro = getAiAssistantIntroMessage();
  assert.match(intro, /v5\.14\.210/);
  assert.match(intro, /아산 배차판·상세배차·변동내역·GLAPS코드·선적관리/);
...
test('채팅 API는 웹 첨부문서 색인을 NAS 원본 파싱처럼 설명하지 않는다', () => {
  ...
  assert.match(summary, /상세배차\/변동내역\/GLAPS코드\/선적관리도 DB 기준/);
