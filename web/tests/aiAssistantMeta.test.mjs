...
test('AI 어시스턴트 버전과 소개는 함수형 메타에서 관리한다', () => {
  assert.equal(getAiAssistantVersion(), 'v5.14.377');

  const intro = getAiAssistantIntroMessage();
  assert.match(intro, /v5\.14\.377/);
...
test('빠른 질문과 가이드는 실제 연결된 DB·웹·안전운임 기능만 노출한다', () => {
  ...
  assert.match(guideText, /대신 운송사 자체의 상차지별 배차수량/);
  assert.match(guideText, /아산 운송내역 데이터베이스 조회/);
  assert.match(guideText, /청구금액 공란 시 실적관리 구간단가 후보/);
...
test('채팅 API는 웹 첨부문서 색인을 NAS 원본 파싱처럼 설명하지 않는다', () => {
  ...
  assert.match(summary, /청구금액 칸이 비면 실적관리 구간단가 후보/);
