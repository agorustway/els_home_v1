...
test('AI 어시스턴트 버전과 소개는 함수형 메타에서 관리한다', () => {
  assert.equal(getAiAssistantVersion(), 'v5.14.301');

  const intro = getAiAssistantIntroMessage();
  assert.match(intro, /v5\.14\.301/);
  assert.match(intro, /아산 배차판·상세배차·변동내역·GLAPS코드·선적관리·실적관리/);
...
test('빠른 질문과 가이드는 실제 연결된 DB·웹·안전운임 기능만 노출한다', () => {
  ...
  assert.match(guideText, /아산 선적관리 DB 조회/);
  assert.match(guideText, /아산 실적관리 DB 조회/);
...
test('채팅 API는 웹 첨부문서 색인을 NAS 원본 파싱처럼 설명하지 않는다', () => {
  ...
  assert.match(summary, /상세배차\/변동내역\/GLAPS코드\/선적관리\/실적관리도 DB 기준/);
