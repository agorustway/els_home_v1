...
test('실시간 마커 표시 좌표는 가까운 도로 경로로만 스냅한다', () => {
  const raw = { lat: 36.80110, lng: 127.00720 };
  const roadPath = [
    { lat: 36.80000, lng: 127.00700 },
    { lat: 36.80150, lng: 127.00700 },
    { lat: 36.80200, lng: 127.00700 },
  ];

  const snapped = snapPointToRoadPath(raw, roadPath);

  assert.equal(snapped.ok, true);
  assert.ok(snapped.distanceKm > 0.01);
  assert.ok(Math.abs(snapped.lng - 127.00700) < 0.00002);
});

test('도로 경로가 원시 좌표에서 멀면 실시간 마커 스냅을 적용하지 않는다', () => {
  const raw = { lat: 36.80110, lng: 127.00720 };
  const farRoadPath = [
    { lat: 36.80110, lng: 127.01000 },
    { lat: 36.80200, lng: 127.01000 },
  ];

  const snapped = snapPointToRoadPath(raw, farRoadPath);

  assert.equal(snapped.ok, false);
  assert.equal(snapped.reason, 'too_far_from_route');
});
