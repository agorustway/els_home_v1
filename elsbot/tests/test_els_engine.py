...
def test_engine_flow():
    ...
    print(f"[TEST] {u_id} 계정으로 엔진 플로우 테스트 시작...")
    
    # 2. 로그인 및 준비 테스트
    driver, error = login_and_prepare(u_id, u_pw, log_callback=print, show_browser=True)
    ...
    try:
        # 3. 조회 테스트 (형이 준 실제 번호)
        test_cns = ["HMMU6915674", "TCKU7261732"]
        for test_cn in test_cns:
            ...
            if grid_text:
                ...
                print(f"[PREVIEW] {grid_text[:300]}...")
            else:
                print(f"[FAIL] {test_cn} 데이터 추출 실패 (아무 응답 없음)")
            
            time.sleep(2) # 간격

    finally:
        print("\n[INFO] 테스트 종료. 5초 후 브라우저를 닫습니다.")
        time.sleep(5)
        driver.quit()
